...
RUN_ID="qgga47ub38w1py_8rtxpro6000_55179e3_diamond_we_full_w8b1_selected_crt22_log15_iter5_3tb_20260619_2309UTC"
LOG_DIR="/workspace/logs/runpod/diamond_we"
BASE="${LOG_DIR}/bench_${RUN_ID}"
TMP_BASE="${BASE}"
LOG="${TMP_BASE}.log"
STATUS="${TMP_BASE}.status"
VRAM="${TMP_BASE}.vram.csv"
VRAM_STATUS="${TMP_BASE}.vram.status"
WRAPPER_OUT="${TMP_BASE}.wrapper.out"
DONE="${TMP_BASE}.done"
  ...
  if [[ -s "${VRAM}" ]]; then
    awk -F, '
      NR>1 {
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", $3);
        used=$3+0;
        if (used > peak) { peak=used; peak_ts=$1; peak_gpu=$2; peak_total=$4; peak_util=$5; }
      }
      END {
...
write_status() {
  ...
  write_vram_status
  {
    ...
    echo "expected_commit=55179e3c72ef743e0794f3e3a88f37c6b9db77dc"
...
echo "expected_commit=55179e3c72ef743e0794f3e3a88f37c6b9db77dc" >> "${STATUS}"
