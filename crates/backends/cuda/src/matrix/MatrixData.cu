...
extern "C" int gpu_matrix_get_format(const GpuMatrix *mat, int *out_format)
{
    if (!mat || !out_format)
        return set_error("invalid gpu_matrix_get_format arguments");
    *out_format = static_cast<int>(mat->format);
    return 0;
}
    ...
    size_t cols)
{
    ...
    // The eager copy follows each limb's ordinary stream and completion event.
    ...
    uint32_t status_binding_index)
{
    ...
    cudaStream_t dispatch_stream = stream;
    if (!dispatch_stream)
    {
