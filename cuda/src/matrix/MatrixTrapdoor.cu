    ...
    GpuMatrix *out)
{
    ...
    if (src->format == PolyFormat::Eval)
    {
        ...
        status = transform_matrix_format_sync(
            tmp_inputs_matrix,
            PolyFormat::Coeff,
            "failed to transform clone matrix to coeff in gpu_matrix_gauss_samp_gq_arb_base");
        if (status != 0)
        {
            cleanup_tmp_inputs();
            return status;
        }
        for (const auto *clone : tmp_inputs_matrix->polys)
        {
    ...
    int transform_status = transform_matrix_format_sync(
        out,
        PolyFormat::Eval,
        "failed to transform output matrix to eval in gpu_matrix_gauss_samp_gq_arb_base");
    ...
    GpuMatrix *out)
{
        ...
        *owned = nullptr;
        if (src->format == PolyFormat::Eval)
        {
            ...
            status = transform_matrix_format_sync(
                *owned,
                PolyFormat::Coeff,
                "failed to transform cloned input matrix to coeff in gpu_matrix_sample_p1_full");
            if (status != 0)
            {
                gpu_matrix_destroy(*owned);
                *owned = nullptr;
            ...
            for (const auto *clone : (*owned)->polys)
            {
                inputs.push_back(clone);
            }
        }
    ...
    status = transform_matrix_format_sync(
        out,
        PolyFormat::Eval,
        "failed to transform output matrix to eval in gpu_matrix_sample_p1_full");
    if (status != 0)
    {
        cleanup();
        return status;
...
namespace
{
    struct GaussSamplingScratch
    {
        int device;
        const uint64_t **d_src;
        uint64_t **d_dst;
        uint64_t *d_tower_moduli;
        uint32_t *d_digit_indices;
        uint32_t *d_tower_indices;
        uint64_t *d_out_moduli;
        size_t src_capacity;
        size_t dst_capacity;
        size_t tower_moduli_capacity;
        size_t digit_indices_capacity;
        size_t tower_indices_capacity;
        size_t out_moduli_capacity;
    };

    GaussSamplingScratch &gauss_sampling_scratch_for_device(int device)
    {
        thread_local std::vector<GaussSamplingScratch> scratches;
        ...
        scratches.push_back(GaussSamplingScratch{
            device,
            nullptr,
            nullptr,
            nullptr,
    ...
    int ensure_gauss_sampling_scratch(
        int device,
        size_t ptr_count,
        size_t job_count,
        const uint64_t ***src_table,
        uint64_t ***dst_table,
        uint64_t **tower_moduli_table,
        uint32_t **digit_indices_table,
        uint32_t **tower_indices_table,
        uint64_t **out_moduli_table)
    {
        if (!src_table || !dst_table || !tower_moduli_table ||
            !digit_indices_table || !tower_indices_table || !out_moduli_table)
        {
            return set_error("invalid output tables in ensure_gauss_sampling_scratch");
        }

        auto &scratch = gauss_sampling_scratch_for_device(device);
        int status = ensure_device_pointer_capacity<const uint64_t *>(
            device,
            &scratch.d_src,
        ...
        status = ensure_device_pointer_capacity<uint64_t *>(
            device,
            &scratch.d_dst,
        ...
        status = ensure_device_pointer_capacity<uint64_t>(
            device,
            &scratch.d_tower_moduli,
            &scratch.tower_moduli_capacity,
            job_count);
        if (status != 0)
        {
            return status;
        }
        status = ensure_device_pointer_capacity<uint32_t>(
            device,
            &scratch.d_digit_indices,
            &scratch.digit_indices_capacity,
            job_count);
        if (status != 0)
        {
            return status;
        ...
        status = ensure_device_pointer_capacity<uint32_t>(
            device,
            &scratch.d_tower_indices,
            &scratch.tower_indices_capacity,
            job_count);
        if (status != 0)
        {
            return status;
        ...
        status = ensure_device_pointer_capacity<uint64_t>(
            device,
            &scratch.d_out_moduli,
            &scratch.out_moduli_capacity,
            job_count);
        if (status != 0)
        {
            return status;
        ...
        *tower_moduli_table = scratch.d_tower_moduli;
        *digit_indices_table = scratch.d_digit_indices;
        *tower_indices_table = scratch.d_tower_indices;
        *out_moduli_table = scratch.d_out_moduli;
        return 0;
    }
    struct SampleP1PointerScratch
    {
        int device;
        const uint64_t **d_a_entries;
        const uint64_t **d_b_entries;
        const uint64_t **d_d_entries;
        ...
        double *col_workspace;
        int64_t *sampled_workspace;
        size_t a_entries_capacity;
        size_t b_entries_capacity;
        size_t d_entries_capacity;
        size_t tp2_entries_capacity;
        size_t sampled_out_capacity;
        size_t cov_workspace_capacity;
        size_t mean_workspace_capacity;
        size_t col_workspace_capacity;
        size_t sampled_workspace_capacity;
    };

    SampleP1PointerScratch &sample_p1_pointer_scratch_for_device(int device)
    {
        thread_local std::vector<SampleP1PointerScratch> scratches;
        for (auto &scratch : scratches)
        {
            if (scratch.device == device)
            {
        ...
        scratches.push_back(SampleP1PointerScratch{
            device,
            nullptr,
            nullptr,
            nullptr,
    ...
    int ensure_sample_p1_pointer_scratch(
        int device,
        size_t mat_entries,
        size_t vec_entries,
        size_t sampled_values,
        const uint64_t ***a_entries_table,
        const uint64_t ***b_entries_table,
        const uint64_t ***d_entries_table,
        const uint64_t ***tp2_entries_table,
        int64_t **sampled_out_table)
    {
        if (!a_entries_table || !b_entries_table || !d_entries_table ||
            !tp2_entries_table || !sampled_out_table)
        {
            return set_error("invalid output tables in ensure_sample_p1_pointer_scratch");
        }

        auto &scratch = sample_p1_pointer_scratch_for_device(device);
        int status = ensure_device_pointer_capacity<const uint64_t *>(
            device,
            &scratch.d_a_entries,
            &scratch.a_entries_capacity,
            mat_entries);
        if (status != 0)
        {
            return status;
        }
        status = ensure_device_pointer_capacity<const uint64_t *>(
            device,
            &scratch.d_b_entries,
            &scratch.b_entries_capacity,
            mat_entries);
        if (status != 0)
        {
            return status;
        ...
        status = ensure_device_pointer_capacity<const uint64_t *>(
            device,
            &scratch.d_tp2_entries,
            &scratch.tp2_entries_capacity,
            vec_entries);
        if (status != 0)
        {
            return status;
        }
        status = ensure_device_pointer_capacity<int64_t>(
            device,
            &scratch.d_sampled_out,
            &scratch.sampled_out_capacity,
            sampled_values);
        if (status != 0)
        {
            return status;
        ...
        *a_entries_table = scratch.d_a_entries;
        *b_entries_table = scratch.d_b_entries;
        *d_entries_table = scratch.d_d_entries;
        *tp2_entries_table = scratch.d_tp2_entries;
        *sampled_out_table = scratch.d_sampled_out;
        return 0;
    }

    int ensure_sample_p1_workspace_buffer_capacity(
        int device,
        ...
        size_t *capacity)
    {
        if (!buffer || !capacity)
        {
            return set_error("invalid workspace buffer arguments in ensure_sample_p1_workspace_buffer_capacity");
        }
        ...
        double *new_buffer = nullptr;
        err = cudaMalloc(&new_buffer, required_count * sizeof(double));
    ...
    int ensure_sample_p1_workspace_i64_buffer_capacity(
        int device,
        size_t required_count,
        int64_t **buffer,
        size_t *capacity)
    {
        if (!buffer || !capacity)
        {
            return set_error("invalid workspace buffer arguments in ensure_sample_p1_workspace_i64_buffer_capacity");
        }
        if (required_count <= *capacity)
        {
            return 0;
        ...
        int64_t *new_buffer = nullptr;
        err = cudaMalloc(&new_buffer, required_count * sizeof(int64_t));
        if (err == cudaErrorMemoryAllocation)
        {
            cudaGetLastError();
            return 2;
    ...
    struct ScatterP1Scratch
    {
        int device;
        uint64_t **d_out_entries;
        int64_t *d_sampled_in;
        size_t out_entries_capacity;
        size_t sampled_in_capacity;
    };

    ScatterP1Scratch &scatter_p1_scratch_for_device(int device)
    {
        thread_local std::vector<ScatterP1Scratch> scratches;
        for (auto &scratch : scratches)
        {
            if (scratch.device == device)
            {
        ...
        scratches.push_back(ScatterP1Scratch{
            device,
            nullptr,
            nullptr,
            0,
    ...
    int ensure_scatter_p1_scratch(
        int device,
        size_t entry_count,
        size_t sampled_count,
        uint64_t ***out_entries_table,
        int64_t **sampled_in_table)
    {
        if (!out_entries_table || !sampled_in_table)
        {
            return set_error("invalid output tables in ensure_scatter_p1_scratch");
        }

        auto &scratch = scatter_p1_scratch_for_device(device);
        int status = ensure_device_pointer_capacity<uint64_t *>(
            device,
            &scratch.d_out_entries,
            &scratch.out_entries_capacity,
            entry_count);
        if (status != 0)
        {
            return status;
        ...
        status = ensure_device_pointer_capacity<int64_t>(
            device,
            &scratch.d_sampled_in,
            &scratch.sampled_in_capacity,
            sampled_count);
        if (status != 0)
        {
            return status;
        ...
        *out_entries_table = scratch.d_out_entries;
        *sampled_in_table = scratch.d_sampled_in;
        return 0;
    }
    ...
    cudaStream_t stream)
{
    ...
    int status = ensure_gauss_sampling_scratch(
        device,
        ptr_count,
        job_count,
        &d_src,
    ...
    std::vector<int64_t> &sampled_out_host)
{
    ...
    int status = ensure_sample_p1_pointer_scratch(
        device_id,
        mat_entries,
        vec_entries,
        total_values,
    ...
    auto &workspace_scratch = sample_p1_pointer_scratch_for_device(device_id);
    auto ensure_workspace = [&](size_t samples) -> int
    {
        if (samples == 0)
        {
            return 2;
        }
        if (samples > std::numeric_limits<size_t>::max() / cov_elems_per_sample ||
            samples > std::numeric_limits<size_t>::max() / vec_elems_per_sample ||
            samples > std::numeric_limits<size_t>::max() / sampled_elems_per_sample)
        {
            return set_error("workspace overflow in matrix_sample_p1_integer_kernel");
        }

        const size_t cov_required = samples * cov_elems_per_sample;
        const size_t vec_required = samples * vec_elems_per_sample;
        const size_t sampled_required = samples * sampled_elems_per_sample;

        int local_status = ensure_sample_p1_workspace_buffer_capacity(
            device_id,
            cov_required,
            &workspace_scratch.cov_workspace,
            &workspace_scratch.cov_workspace_capacity);
        if (local_status != 0)
        {
            return local_status;
        }
        local_status = ensure_sample_p1_workspace_buffer_capacity(
            device_id,
            vec_required,
            &workspace_scratch.mean_workspace,
            &workspace_scratch.mean_workspace_capacity);
        if (local_status != 0)
        {
            return local_status;
        }
        local_status = ensure_sample_p1_workspace_buffer_capacity(
            device_id,
            vec_required,
            &workspace_scratch.col_workspace,
            &workspace_scratch.col_workspace_capacity);
        if (local_status != 0)
        {
            return local_status;
        }
        local_status = ensure_sample_p1_workspace_i64_buffer_capacity(
            device_id,
            sampled_required,
            &workspace_scratch.sampled_workspace,
            &workspace_scratch.sampled_workspace_capacity);
        if (local_status != 0)
        {
            return local_status;
        }
    ...
    while (true)
    {
        const int workspace_status = ensure_workspace(chunk_samples);
        if (workspace_status == 0)
        {
            break;
        }
        if (workspace_status != 2)
        {
    ...
    int device_id)
{
    ...
    int status = ensure_scatter_p1_scratch(
        device_id,
        out_entries.size(),
        total,
        &d_out_entries,
