...
const PARALLEL_PRODUCT_PAIR_BATCH: usize = 256;
...
impl<'a> Normalizer<'a> {
    ...
    fn compute_use_counts(&mut self, root: ExprId) -> Result<BTreeSet<ExprId>, NormalizeError> {
        ...
        *self.remaining_uses.entry(root).or_default() += 1;
        ...
        let scalar_action_candidates = scalar_action_candidates
            .into_iter()
            .filter(|expression| {
            ...
            .collect::<BTreeSet<_>>();
        let mut scalar_actions_by_matrix = BTreeMap::<ExprId, BTreeSet<ExprId>>::new();
        for expression in &scalar_action_candidates {
            let node = self.expressions.node(*expression)?;
            let mut matrix_input = None;
            for input in &node.inputs {
                let ResolvedValueType::Matrix(matrix) = self.expressions.value_type(*input)? else {
                    continue;
                };
                if matrix.rows != 1 || matrix.columns != 1 {
                    matrix_input = Some(*input);
                }
            }
            let Some(matrix_input) = matrix_input else { continue };
            scalar_actions_by_matrix.entry(matrix_input).or_default().insert(*expression);
        }
        let mut admitted_scalar_actions = BTreeSet::new();
        let mut barrier_pending = Vec::new();
        for (matrix, actions) in scalar_actions_by_matrix {
            if actions.len() >= 2 {
                admitted_scalar_actions.extend(actions);
                barrier_pending.push(matrix);
            }
        }
        // Stop product deferral at the first multiply below an admitted scalar action's matrix
        // operand so the eager frontier commits at a node boundary where normal monomial GC
        // remains available. Nonlinear operators other than multiply are already eager boundaries.
        // The barrier is global: a shared expression never has two edge-dependent representations.
        let mut barrier_visited = BTreeSet::new();
        let mut product_frontier = BTreeSet::new();
        while let Some(expression) = barrier_pending.pop() {
            if !barrier_visited.insert(expression) {
                continue;
            }
            let node = self.expressions.node(expression)?;
            match node.operator {
                ValueOperator::Matrix(MatrixOperation::Add | MatrixOperation::Subtract) => {
                    barrier_pending.extend(node.inputs.iter().copied());
                }
                ValueOperator::Matrix(MatrixOperation::Multiply) => {
                    product_frontier.insert(expression);
                }
                _ => {}
            }
        }
        for expression in &product_frontier {
            candidates.remove(expression);
            gadget_candidates.remove(expression);
            admitted_scalar_actions.remove(expression);
        }
        let (eligible, _) = self.propagate_product_eligibility(&candidates, &real_consumers)?;
        self.deferred_products = eligible;
        self.deferred_scalar_actions = admitted_scalar_actions;
...
mod tests {
    ...
    fn single_scalar_action_matches_eager_on_both_sides_without_deferral() {
        for scalar_on_left in [false, true] {
            ...
            assert_eq!(descriptor.central_factors.len(), 1);
            assert_eq!(descriptor.ordered_factors.len(), 1);
            assert_eq!(counters.scalar_action_plans_created, 0);
            assert_eq!(counters.scalar_action_streamed_executions, 0);
    ...
    fn deferred_scalar_actions_share_one_additive_matrix_output_and_cancel() {
        for subtract_last in [false, true] {
            ...
            assert_eq!(normalizer.product_plan_counters.scalar_action_zero_weight_skips, 0);
    ...
    #[test]
    fn two_distinct_admitted_scalar_actions_cancel_without_output_underflow() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
        let matrix_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 2).unwrap();
        let a = matrix_source(&mut expressions, "cancel-action-a", matrix_type.clone(), None);
        let b = matrix_source(&mut expressions, "cancel-action-b", matrix_type, None);
        let shared = expressions.intern_matrix_transform(MatrixOperation::Add, &[a, b]).unwrap();
        let scalar = matrix_source(&mut expressions, "cancel-action-scalar", scalar_type, None);
        let negated =
            expressions.intern_matrix_transform(MatrixOperation::Negate, &[scalar]).unwrap();
        let equivalent =
            expressions.intern_matrix_transform(MatrixOperation::Negate, &[negated]).unwrap();
        let left = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar])
            .unwrap();
        let right = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[shared, equivalent])
            .unwrap();
        let root =
            expressions.intern_matrix_transform(MatrixOperation::Subtract, &[left, right]).unwrap();
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
        ...
        assert_eq!(normalizer.product_plan_counters.scalar_action_plans_created, 2);
        assert_eq!(normalizer.product_plan_counters.scalar_action_streamed_executions, 2);
    }

    #[test]
    fn scalar_action_admission_requires_distinct_shared_additive_consumers() {
        let run = |case: u8| {
            let mut expressions = ExprArena::new();
            let mut programs = ProgramArena::new();
            let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
            let matrix_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 2).unwrap();
            let a = matrix_source(&mut expressions, "fanout-a", matrix_type.clone(), None);
            let b = matrix_source(&mut expressions, "fanout-b", matrix_type, None);
            let shared =
                expressions.intern_matrix_transform(MatrixOperation::Add, &[a, b]).unwrap();
            let scalar_a =
                matrix_source(&mut expressions, "fanout-scalar-a", scalar_type.clone(), None);
            let scalar_b = matrix_source(&mut expressions, "fanout-scalar-b", scalar_type, None);
            let action_a = expressions
                .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar_a])
                .unwrap();
            let action_b = expressions
                .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar_b])
                .unwrap();
            let root = match case {
                0 => expressions
                    .intern_matrix_transform(MatrixOperation::Add, &[action_a, action_b])
                    .unwrap(),
                1 => expressions
                    .intern_matrix_transform(MatrixOperation::Add, &[action_a, action_a])
                    .unwrap(),
                _ => expressions
                    .intern_matrix_transform(MatrixOperation::Add, &[action_a, shared])
                    .unwrap(),
            };
            let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
            let mut normalizer =
                Normalizer::new(&mut expressions, &programs, &facts, &mut monomials).unwrap();
            normalizer.compute_use_counts(semantic.expression()).unwrap();
            (
                normalizer.deferred_scalar_actions.contains(&action_a),
                normalizer.deferred_scalar_actions.contains(&action_b),
            )
        };
        assert_eq!(run(0), (true, true));
        assert_eq!(run(1), (false, false));
        assert_eq!(run(2), (false, false));
    }

    #[test]
    fn scalar_action_shared_direct_product_is_admitted_and_forced_eager() {
        for matrix_is_product in [false, true] {
            let mut expressions = ExprArena::new();
            let mut programs = ProgramArena::new();
            let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
            let matrix_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 2).unwrap();
            let a = matrix_source(&mut expressions, "direct-fanout-a", matrix_type.clone(), None);
            let b = matrix_source(&mut expressions, "direct-fanout-b", matrix_type, None);
            let matrix = if matrix_is_product {
                expressions.intern_matrix_transform(MatrixOperation::Multiply, &[a, b]).unwrap()
            } else {
                a
            };
            let scalar_a = matrix_source(
                &mut expressions,
                "direct-fanout-scalar-a",
                scalar_type.clone(),
                None,
            );
            let scalar_b =
                matrix_source(&mut expressions, "direct-fanout-scalar-b", scalar_type, None);
            let action_a = expressions
                .intern_matrix_transform(MatrixOperation::Multiply, &[matrix, scalar_a])
                .unwrap();
            let action_b = expressions
                .intern_matrix_transform(MatrixOperation::Multiply, &[matrix, scalar_b])
                .unwrap();
            let root = expressions
                .intern_matrix_transform(MatrixOperation::Add, &[action_a, action_b])
                .unwrap();
            ...
            let mut normalizer =
                Normalizer::new(&mut expressions, &programs, &facts, &mut monomials).unwrap();
            normalizer.compute_use_counts(semantic.expression()).unwrap();
            assert!(normalizer.deferred_scalar_actions.contains(&action_a));
            assert!(normalizer.deferred_scalar_actions.contains(&action_b));
            if matrix_is_product {
                assert!(!normalizer.deferred_products.contains(&matrix));
                assert!(!normalizer.deferred_gadget_products.contains(&matrix));
            }
        }
    }

    #[test]
    fn scalar_action_backward_barrier_forces_only_first_product_frontiers_eager() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
        let matrix_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 2).unwrap();
        let a = matrix_source(&mut expressions, "barrier-a", matrix_type.clone(), None);
        let b = matrix_source(&mut expressions, "barrier-b", matrix_type.clone(), None);
        let c = matrix_source(&mut expressions, "barrier-c", matrix_type.clone(), None);
        let d = matrix_source(&mut expressions, "barrier-d", matrix_type.clone(), None);
        let e = matrix_source(&mut expressions, "barrier-e", matrix_type.clone(), None);
        let frontier =
            expressions.intern_matrix_transform(MatrixOperation::Multiply, &[a, b]).unwrap();
        let shared =
            expressions.intern_matrix_transform(MatrixOperation::Add, &[frontier, c]).unwrap();
        let scalar_a =
            matrix_source(&mut expressions, "barrier-scalar-a", scalar_type.clone(), None);
        let scalar_b = matrix_source(&mut expressions, "barrier-scalar-b", scalar_type, None);
        let action_a = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar_a])
            .unwrap();
        let action_b = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar_b])
            .unwrap();
        let actions = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[action_a, action_b])
            .unwrap();
        let unrelated =
            expressions.intern_matrix_transform(MatrixOperation::Multiply, &[d, e]).unwrap();
        let combined = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[actions, unrelated])
            .unwrap();
        let root = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[combined, frontier])
            .unwrap();
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
        let mut normalizer =
            Normalizer::new(&mut expressions, &programs, &facts, &mut monomials).unwrap();
        ...
        assert!(normalizer.deferred_scalar_actions.contains(&action_a));
        assert!(normalizer.deferred_scalar_actions.contains(&action_b));
        assert!(!normalizer.deferred_products.contains(&frontier));
        assert!(!normalizer.deferred_gadget_products.contains(&frontier));
        assert!(normalizer.deferred_products.contains(&unrelated));
    }

    #[test]
    fn scalar_action_backward_barrier_excludes_typed_gadget_frontier() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let input_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 4).unwrap();
        let decomposition_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 4, 4).unwrap();
        let gadget_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 4).unwrap();
        let (gadget, decomposition, _) = gadget_product(
            &mut expressions,
            false,
            4,
        ...
        let frontier = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[gadget, decomposition])
            .unwrap();
        let other =
            matrix_source(&mut expressions, "typed-barrier-other", input_type.clone(), None);
        let shared =
            expressions.intern_matrix_transform(MatrixOperation::Add, &[frontier, other]).unwrap();
        let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
        let scalar_a =
            matrix_source(&mut expressions, "typed-barrier-scalar-a", scalar_type.clone(), None);
        let scalar_b = matrix_source(&mut expressions, "typed-barrier-scalar-b", scalar_type, None);
        let action_a = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[shared, scalar_a])
            .unwrap();
        let action_b = expressions
