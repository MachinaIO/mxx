...
impl TallRunMode {
    /// The application-agnostic simulator has been retired. Until Tall supplies its own
    /// kernel-checked certificate, neither parameter acceptance nor comparison against a
    /// proved noise bound is available. Graph inspection, selected-parameter cost estimates,
    /// and the independent exact noiseless round trip do not claim that acceptance.
    fn require_available(self) -> Result<(), String> {
        match self {
            Self::Simulation | Self::Benchmark | Self::Full => Err(format!(
                "Tall {self:?} mode is unavailable: the application-agnostic operational-noise \
                 simulator was retired; a Tall-specific Lean correctness certificate is required \
                 before parameter simulation, automatic benchmark selection, or noisy full \
                 execution can run. Available modes are graph, zero-noise, and benchmark-selected \
                 (explicit parameters; security and cost estimation only)."
            )),
            Self::Graph | Self::ZeroNoise | Self::BenchmarkSelected => Ok(()),
        }
    }
...
fn prepare_selected_benchmark_candidate(config: &TestConfig) -> Result<PreparedCandidate, String> {
    ...
    info!(
        ...
        "preparing explicitly selected parameters for security and cost estimation only; correctness is unverified"
    );
...
#[test]
fn certificate_dependent_modes_fail_before_execution() {
    for mode in [TallRunMode::Simulation, TallRunMode::Benchmark, TallRunMode::Full] {
        let error = mode.require_available().expect_err("Tall certificate is not implemented");
        assert!(error.contains("Tall-specific Lean correctness certificate"));
    }
    for mode in [TallRunMode::Graph, TallRunMode::ZeroNoise, TallRunMode::BenchmarkSelected] {
        mode.require_available().expect("mode does not claim correctness acceptance");
    }
}
...
#[test]
#[ignore = "explicit graph, noiseless GPU round trip, or selected-parameter benchmark; certificate-dependent modes are unavailable"]
fn test_gpu_tall_bgg_nested_rns_modq_arithmetic() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::from_default_env())
        .try_init();
    let requested_mode = TallRunMode::from_env().expect("valid Tall execution mode");
    requested_mode.require_available().expect("available Tall execution mode");
    ...
    let (config, selected) = match requested_mode {
        ...
        TallRunMode::BenchmarkSelected => {
            let selected = prepare_selected_benchmark_candidate(&requested_config)
                .expect("explicitly selected benchmark parameters");
            (requested_config, selected)
        }
        TallRunMode::Simulation | TallRunMode::Benchmark | TallRunMode::Full => {
            unreachable!("certificate-dependent modes were rejected before graph construction")
        }
    };
