...
fn decode_mod_p_from_plaintext(plaintext: &DCRTPoly, p: u64) -> u64 {
    let coeff = plaintext
        .coeffs()
        .into_iter()
        .next()
...
fn find_crt_depth_for_modp_chain() -> (usize, DCRTPolyParams, PolyCircuit<DCRTPoly>, BigUint) {
    ...
    for crt_depth in 1..=MAX_CRT_DEPTH {
        ...
        let circuit = build_modp_chain_circuit(&params, P);
...
async fn test_commit_modp_chain_rounding() {
    ...
    let expected_plaintext = DCRTPoly::from_biguint_to_constant(&params, expected.clone());
    let plaintext =
    ...
    let expected_times_gadget = s_vec.clone() *
        (DCRTPolyMatrix::gadget_matrix(&params, d_secret) * expected_plaintext.clone());
