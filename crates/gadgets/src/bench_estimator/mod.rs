...
pub use bgg_pubkey::*;
#[cfg(feature = "gpu")]
pub use gpu::benchmark_gate_operation;
#[cfg(feature = "gpu")]
pub use gpu::*;
pub use naive_vec::*;
pub use native::NativePolyMatrixBenchEstimator;
#[cfg(feature = "gpu")]
pub use native_gpu::GpuDCRTPolyMatrixNativeBenchEstimator;
...
pub struct BenchOperationMeasurement {
    pub time: f64,
    pub peak_vram: usize,
}
...
impl CircuitBenchEstimate {
    pub fn new(total_time: f64, latency: f64) -> Self {
        let total_time_nanos = seconds_to_nanos(total_time);
        Self::from_nanos(total_time_nanos, latency)
    }

    pub fn from_nanos(total_time: BigUint, latency: f64) -> Self {
    ...
    pub fn with_max_parallelism<T: Into<BigUint>>(mut self, max_parallelism: T) -> Self {
        self.max_parallelism = max_parallelism.into();
        self
    }

    #[cfg(feature = "gpu")]
    pub fn with_peak_vram(mut self, peak_vram: usize) -> Self {
        self.peak_vram = peak_vram;
        self
    }

    #[cfg(not(feature = "gpu"))]
    pub fn with_peak_vram(self, _peak_vram: usize) -> Self {
        self
    }
...
impl CircuitBenchSummary {
    pub fn new<T: Into<BigUint>>(total_time: f64, latency: f64, max_parallelism: T) -> Self {
        Self::from_nanos(seconds_to_nanos(total_time), latency, max_parallelism)
    }

    pub fn from_nanos<T: Into<BigUint>>(
        total_time: BigUint,
        latency: f64,
        max_parallelism: T,
    ) -> Self {
...
/// representative unit.
pub fn scale_independent_estimate(
...
pub fn scale_independent_summary(
...
pub fn measure_bench_operation<R, F>(iterations: usize, mut op: F) -> f64
where
    F: FnMut() -> R,
{
...
#[cfg(not(feature = "gpu"))]
pub fn benchmark_gate_operation<R, F>(iterations: usize, op: F) -> BenchOperationMeasurement
where
    F: FnMut() -> R,
{
...
pub fn estimate_public_key_circuit_bench_with_aux<E, B>(
    estimator: &B,
