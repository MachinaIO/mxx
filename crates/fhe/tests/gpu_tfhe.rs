...
use mxx_fhe::{FheCommonParams, TfheParams};
use mxx_ir_core::{ParamEnv, ValidatedGraph, node::SampleRange};
use mxx_primitives::poly::{
    PolyParams,
    dcrt::{gpu::GpuDCRTPolyParams, params::DCRTPolyParams},
};
use mxx_runtime::{
    Backend, GpuExecutionPlan, GpuRuntime, MemoryArtifactStore, RuntimeValue,
...
type GpuValues = BTreeMap<String, RuntimeValue<GpuDcrtBackend>>;
...
const RING_MODULUS: u64 = RING_MODULI[0] * RING_MODULI[1];
...
fn gpu_parameters(tfhe: &TfheParams) -> Vec<GpuDCRTPolyParams> {
    ...
    for ring in tfhe.runtime_parameters() {
...
fn tfhe_ring() -> Ring {
    Ring::new(BigInt::from(RING_MODULUS), RING_DIMENSION)
}
...
fn integer_output(runtime: &mut GpuRuntime, outputs: &GpuValues, name: &str) -> BigInt {
    match &outputs[name] {
        RuntimeValue::Int(value) => value.clone(),
        RuntimeValue::NativeInteger(value) => BigInt::from(*value),
        RuntimeValue::IntegerValues(values) => runtime
            .backend_mut()
            .integer_values_to_host(values)
            .expect("download integer output")
            .into_iter()
            .next()
            .expect("one integer output"),
        _ => panic!("{name} is an integer output"),
    }
}
...
fn verify_hash_domains(runtime: &mut GpuRuntime, store: &mut MemoryArtifactStore) {
    ...
    let graph =
        context.build().unwrap().validate(&ParamEnv::default()).expect("valid hash-domain graph");
    let key_a = fresh_hash_key();
    let planning_inputs =
        BTreeMap::from([("domain_probe_key".to_owned(), RuntimeValue::Bytes(key_a.clone()))]);
...
fn execute_hash_probe(
    ...
    let inputs = BTreeMap::from([("domain_probe_key".to_owned(), RuntimeValue::Bytes(key))]);
    let outputs = runtime
        .execute(plan, inputs, store, fresh_nonce())
        .expect("execute hash-domain probe")
        .outputs;
...
fn integer_family_output(runtime: &mut GpuRuntime, outputs: &GpuValues, name: &str) -> Vec<BigInt> {
    let RuntimeValue::IntegerValues(values) = &outputs[name] else {
        panic!("{name} is a packed integer-family output")
    };
    runtime
        .backend_mut()
        .integer_values_to_host(values)
...
impl TimedStage {
    fn new(graph: BuiltGraph) -> Self {
        Self {
            validated: Some(graph.validate(&ParamEnv::default()).expect("valid TFHE graph")),
            plan: None,
        }
    }
...
fn decrypt_bit(
    ...
    integer_output(runtime, &result.outputs, "bit")
}
...
fn encrypt_bit(
    ...
    plan: &mut GpuExecutionPlan,
    store: &mut MemoryArtifactStore,
    lwe_secret: &RuntimeValue<GpuDcrtBackend>,
    value: bool,
) -> GpuValues {
    let inputs = BTreeMap::from([
        ("lwe_sk".to_owned(), lwe_secret.clone()),
        ("message".to_owned(), RuntimeValue::Int(BigInt::from(u8::from(value)))),
        ("encryption_hash_key".to_owned(), RuntimeValue::Bytes(fresh_hash_key())),
    ]);
    runtime
        .execute(plan, inputs, store, fresh_nonce())
        .expect("encrypt noisy TFHE input bit")
        .outputs
}
...
fn test_gpu_tfhe_noisy_nand_and_repeated_gates() {
    ...
    let keygen_graph_built = DslContext::new("gpu-tfhe-keygen")
        ...
        .unwrap();
    let keygen_graph =
        keygen_graph_built.validate(&ParamEnv::default()).expect("valid key generation graph");
    ...
    verify_hash_domains(&mut runtime, &mut store);
    let keygen_inputs =
        BTreeMap::from([("keygen_hash_key".to_owned(), RuntimeValue::Bytes(fresh_hash_key()))]);
    let mut keygen_plan =
        runtime.plan(keygen_graph, &keygen_inputs).expect("plan TFHE key generation");
    let key_outputs = runtime
    ...
    for (left_bit, right_bit) in [(false, false), (false, true), (true, false), (true, true)] {
        ...
        if decrypt_stage.plan.is_none() {
            ...
            decrypt_stage.plan =
                Some(runtime.plan(validated, &decrypt_inputs).expect("plan TFHE decryption"));
        }
        ...
        if left_bit && right_bit {
            last_truth_table_result = Some(switched_outputs);
