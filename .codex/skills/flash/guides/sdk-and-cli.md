# Runpod Flash SDK and CLI Examples

Consult the relevant section only. Verify signatures, limits, and available resource enums against the installed SDK and CLI before execution. Constructor options illustrate alternatives; select the set appropriate to the chosen mode. Cloud calls in examples can provision resources or incur cost.
...
Endpoint(
    ...
    # Alternative to gpu above: gpu=[GpuGroup.ADA_24, GpuGroup.AMPERE_80]
    # Alternative to gpu: cpu=CpuInstanceType.CPU5C_4_8
    workers=5,                             # shorthand for (0, 5)
    # Alternative to workers above: workers=(1, 5)
