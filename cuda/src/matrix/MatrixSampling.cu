...
namespace
{
    ...
    int ensure_sampling_pointer_capacity(
        int device,
        PtrType **ptr,
        size_t *capacity,
    ...
    struct MultiLimbSamplingScratch
    {
        int device;
        uint64_t **d_dst;
        uint64_t *d_moduli;
        uint32_t *d_limb_indices;
        size_t dst_capacity;
        size_t moduli_capacity;
        size_t limb_indices_capacity;
    };

    MultiLimbSamplingScratch &multi_limb_sampling_scratch_for_device(int device)
    {
        thread_local std::vector<MultiLimbSamplingScratch> scratches;
        ...
        scratches.push_back(MultiLimbSamplingScratch{
            device,
            nullptr,
            nullptr,
            nullptr,
    ...
    {
        if (!dst_table || !moduli_table || !limb_indices_table)
        {
            return set_error("invalid output tables in ensure_multi_limb_sampling_scratch");
        }

        auto &scratch = multi_limb_sampling_scratch_for_device(device);
        int status = ensure_sampling_pointer_capacity<uint64_t *>(
            device,
            &scratch.d_dst,
            &scratch.dst_capacity,
            ptr_count,
            "invalid scratch capacity in ensure_multi_limb_sampling_scratch");
        if (status != 0)
        {
            return status;
        }
        status = ensure_sampling_pointer_capacity<uint64_t>(
            device,
            &scratch.d_moduli,
            &scratch.moduli_capacity,
            limb_count,
            "invalid scratch capacity in ensure_multi_limb_sampling_scratch");
        if (status != 0)
        {
        ...
        status = ensure_sampling_pointer_capacity<uint32_t>(
            device,
            &scratch.d_limb_indices,
            &scratch.limb_indices_capacity,
            limb_count,
            "invalid scratch capacity in ensure_multi_limb_sampling_scratch");
        if (status != 0)
        {
        ...
        *moduli_table = scratch.d_moduli;
        *limb_indices_table = scratch.d_limb_indices;
        return 0;
    }
} // namespace
    ...
    uint64_t seed)
{
    ...
    if (out->limb_u64_tables.size() < static_cast<size_t>(level + 1))
    {
        return set_error("unexpected limb table size in gpu_matrix_sample_distribution");
    }
    ...
    for (int limb = 0; limb <= level; ++limb)
    {
        ...
        int status = get_matrix_u64_limb_table(
            ...
            limb,
            &table,
            "unsupported limb type in gpu_matrix_sample_distribution");
        if (status != 0)
        {
            return status;
        }
    ...
    return transform_matrix_format_sync(
        out,
        PolyFormat::Eval,
        "failed to transform output matrix to eval in gpu_matrix_sample_distribution");
}
