...
const ERROR_NORM_EXPR_PAR_BATCH_SIZE: usize = 256;
const ERROR_NORM_CALL_COMMIT_BATCH_SIZE: usize = 128;
const ERROR_NORM_SUMMED_CALL_COMMIT_BATCH_SIZE: usize = 128;
const ERROR_NORM_SUMMED_INNER_REDUCE_BATCH_SIZE: usize = 512;
const ERROR_NORM_SUMMED_INNER_REDUCE_LOG_INTERVAL: usize = 256;
static ERROR_NORM_SUMMARY_BUILD_CONTEXT_COUNTER: AtomicUsize = AtomicUsize::new(1);
...
fn error_norm_summary_cache_has_ready_entry(
...
impl PolyCircuit<DCRTPoly> {
    ...
    fn build_error_norm_sub_circuit_summary_direct_with_cache<P: AffinePltEvaluator>(
        ...
        let build_summary_locally = || {
            let sub_circuit = self.registered_sub_circuit_ref(sub_circuit_id);
            Arc::new(sub_circuit.as_ref().build_sub_circuit_summary(
                sub_circuit_id,
                one_error,
        ...
        if let Some(existing_summary) = summary_cache.get(&cache_key) {
            let summary = Arc::clone(existing_summary.value());
            debug!(
        ...
        let summary = build_summary_locally();
        match summary_cache.entry(cache_key) {
            Entry::Occupied(entry) => {
                let summary = Arc::clone(entry.get());
                debug!(
    ...
    fn build_prepared_error_norm_sub_circuit_summaries<P: AffinePltEvaluator>(
        ...
        let cached_unique_count = unique_requests
            ...
            .filter(|(sub_circuit_id, _, materialized_input_plaintext_norms)| {
                ...
                error_norm_summary_cache_has_ready_entry(summary_cache, &cache_key)
            })
            .count();
        debug!(
        ...
        let mut unique_summaries = Vec::with_capacity(unique_request_count);
        for (unique_idx, (sub_circuit_id, param_bindings, materialized_input_plaintext_norms)) in
            unique_requests.into_iter().enumerate()
        {
            ...
            let build_context_id =
                ERROR_NORM_SUMMARY_BUILD_CONTEXT_COUNTER.fetch_add(1, Ordering::Relaxed);
    ...
    fn collect_error_norm_plaintext_norms_for_summary_inputs_direct<P: AffinePltEvaluator>(
        ...
        let mut prefix_norms = shared_prefix
            .iter()
    ...
    fn build_sub_circuit_summary<P: AffinePltEvaluator>(
        ...
        ) in execution_layers.into_iter().enumerate()
        {
                ...
                summed_sub_circuit_call_ids.chunks(ERROR_NORM_SUMMED_CALL_COMMIT_BATCH_SIZE)
            {
                         ...
                         output_gate_ids| {
                                ...
                                (0..output_range.len()).step_by(ERROR_NORM_OUTPUT_BATCH_SIZE)
                            {
                                ...
                                debug!(
                                    ...
                                    level_idx,
                                    summed_call_id,
                                    output_chunk_range.start().0,
                                    output_chunk_range.end().0,
                                    output_chunk_range.len(),
                                    finished_call,
                                );
                                self.store_error_norm_summary_expr_batch(
                                    output_chunk_range.gate_ids().zip(outputs.into_iter()),
                                    &output_positions,
                                    &remaining_use_count,
                                    &mut gate_exprs,
                                    &mut final_output_exprs,
                ...
                if log_large_summary &&
                    (prepared_summed_call_count % (ERROR_NORM_SUMMED_CALL_COMMIT_BATCH_SIZE * 8) ==
                        0 ||
                        prepared_summed_call_count == summed_sub_circuit_call_ids.len())
                {
                    debug!(
                        "error-norm summary summed-sub-call stream batch finished sub_circuit_id={} level={} processed_summed_calls={} total_summed_calls={} elapsed_ms={}",
                        sub_circuit_id,
                        level_idx,
                        prepared_summed_call_count,
                        summed_sub_circuit_call_ids.len(),
