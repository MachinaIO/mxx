...
// The plan is deliberately opaque at the C ABI boundary. Its metadata is
// prepared before capture and retained until the caller destroys the plan;
// graph replay only changes the source/output descriptor arguments.
...
namespace
{
        ...
        bool negative, uint64_t *quotient)
    {
        ...
        for (int word = 0; word < metadata.divisor_word_count; ++word)
            divisor[word] = metadata.divisor_words[word];
        ...
        size_t coefficient_count, size_t ring_dimension)
    {
        ...
        divide_centered_words(value, *metadata, negative, quotient);
    ...
    uint32_t output_binding_index)
{
    ...
    if (restore_evaluation)
    {
        ...
        if (error != cudaSuccess) return set_error(error);
        const size_t copy_sizes[] = {sizeof(void *), sizeof(void *), sizeof(size_t), sizeof(size_t), sizeof(size_t)};
        const MxxGraphPatch source_patch = {
            nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0, 0, sizeof(void *), source_binding_index, 0};
        status = mxx_graph_register_kernel_update_for_stream(source->ctx, reinterpret_cast<void *>(stream),
            copy_sizes, std::size(copy_sizes), &source_patch, 1);
        if (status != 0) return status;
        for (size_t limb = 0; limb < plan->source_count; ++limb)
        {
    ...
    const size_t argument_sizes[] = {
        sizeof(input.device_descriptors), sizeof(output.device_descriptors),
        sizeof(plan->device_metadata), sizeof(size_t), sizeof(size_t), sizeof(bool),
    };
    const MxxGraphPatch patches[] = {
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0, 0, sizeof(void *), source_binding_index, 0},
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 1, 0, sizeof(void *), output_binding_index, 0},
    };
    const int registered = mxx_graph_register_kernel_update_for_stream(
        source->ctx, reinterpret_cast<void *>(stream), argument_sizes, std::size(argument_sizes),
        coefficients ? patches + 1 : patches, coefficients ? 1 : std::size(patches));
    if (status == 0) status = registered;
    for (size_t limb = 0; limb < plan->source_count; ++limb)
    {
    ...
    uint32_t output_binding_index)
{
    ...
    // The inverse transform belongs to the graph: converting only the capture
    // exemplar would bind evaluation-domain replay inputs to a coefficient kernel.
    // This temporary is allocated and freed in the same capture, never retained
    // as a plan-owned payload and never exposed as a boundary output.
    std::unique_ptr<GpuMatrix, decltype(&gpu_matrix_destroy)> coefficients(nullptr, gpu_matrix_destroy);
    const auto *source_descriptors = input.device_descriptors;
    ...
    const size_t argument_sizes[] = {
        sizeof(input.device_descriptors), sizeof(output.device_descriptors),
        sizeof(plan->device_metadata), sizeof(size_t), sizeof(size_t),
    };
    const MxxGraphPatch patches[] = {
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0, 0, sizeof(void *), source_binding_index, 0},
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 1, 0, sizeof(void *), output_binding_index, 0},
    ...
    const uint64_t *plaintext_modulus_words, size_t plaintext_modulus_word_count)
{
    ...
    int status = matrix_limb_stream(out, out->ctx->limb_gpu_ids[0], &stream);
    if (status != 0) return status;
    const bool capture_dispatch = matrix_stream_is_capturing(stream);
    for (size_t limb = 0; !capture_dispatch && limb < source_count; ++limb)
    {
        status = matrix_wait_limb_stream(source, source->ctx->limb_gpu_ids[limb], output.device, stream, false, true);
        if (status != 0) return status;
    }
    for (size_t limb = 0; !capture_dispatch && limb < target_count; ++limb)
    {
        status = matrix_wait_limb_stream(out, out->ctx->limb_gpu_ids[limb], output.device, stream);
        if (status != 0) return status;
    }
    // All scalar CRT metadata is immutable and fits in the kernel argument
    // buffer. Passing it by value avoids pinned allocation and H2D metadata
    // uploads during capture; only the two descriptor fields are rebound.
    ...
    error = cudaGetLastError();
    if (error != cudaSuccess) return set_error(error);
    const size_t argument_sizes[] = {sizeof(BlockModSwitchMetadata), sizeof(size_t), sizeof(size_t)};
    const uint32_t descriptor_binding_base = static_cast<uint32_t>(2 * source_count);
    const MxxGraphPatch patches[] = {
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0,
         offsetof(BlockModSwitchMetadata, source), sizeof(void *), descriptor_binding_base, 0},
        {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0,
         offsetof(BlockModSwitchMetadata, output) + offsetof(CrtOutputMetadata, descriptors),
         sizeof(void *), descriptor_binding_base + 1, 0},
    };
    ...
    size_t reconstruction_stride)
{
    ...
    for (size_t level = 0; !capture_dispatch && level < level_count; ++level)
    ...
    // without pinned metadata or pointer-bearing H2D transfers during capture.
    ...
    for (size_t level = 0; status == 0 && level < level_count; ++level)
    {
        ...
        if (error != cudaSuccess) status = set_error(error);
        if (status != 0) break;
        const size_t argument_sizes[] = {sizeof(CrtLevelMetadata), sizeof(CrtOutputMetadata),
            sizeof(bool), sizeof(size_t), sizeof(size_t)};
        const MxxGraphPatch patches[] = {
            {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 0,
                offsetof(CrtLevelMetadata, descriptors), sizeof(void *), static_cast<uint32_t>(level), 0},
            {nullptr, MXX_GRAPH_PATCH_KERNEL_ARGUMENT_FIELD, 1,
                offsetof(CrtOutputMetadata, descriptors), sizeof(void *), static_cast<uint32_t>(level_count), 0},
        };
        status = mxx_graph_register_kernel_update_for_stream(out->ctx,
            reinterpret_cast<void *>(stream), argument_sizes, std::size(argument_sizes),
            patches, std::size(patches));
    }
    if (!capture_dispatch)
    {
