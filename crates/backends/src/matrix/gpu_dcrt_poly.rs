    ...
    poly::{
        ...
        dcrt::{
            gpu::{
                ...
                GpuMatrixBindingLimbRaw, GpuMatrixLaneLimbLayoutRaw,
                GpuMatrixLanePhysicalLayoutRaw, GpuMatrixOpaque, GpuModulusConversionPlan,
                GpuModulusConversionPlanOpaque, GpuNativeEvent, GpuNativeGraphError,
                GpuNativeLaunchStream, GpuP1CovarianceCacheOpaque, GpuRngSeed,
                ...
                gpu_matrix_binding_limb, gpu_matrix_binding_limb_count,
                ...
                gpu_matrix_copy, gpu_matrix_copy_block, gpu_matrix_copy_block_on_stream,
                ...
                gpu_matrix_decompose_base, gpu_matrix_decompose_base_small,
                gpu_matrix_decompose_coeff_graph, gpu_matrix_destroy,
                ...
                gpu_matrix_gauss_samp_gq_arb_base_into, gpu_matrix_get_format, gpu_matrix_intt_all,
                gpu_matrix_intt_batch, gpu_matrix_lane_physical_layout,
                gpu_matrix_load_compact_bytes, gpu_matrix_load_rns_batch,
                gpu_matrix_modulus_conversion_prepare, gpu_matrix_modulus_conversion_submit,
                ...
                gpu_matrix_mul_vertical_pair, gpu_matrix_negate_batch,
                gpu_matrix_negate_lane_batch_layout, gpu_matrix_ntt_all,
                gpu_matrix_ntt_in_place_batch, gpu_matrix_preimage_add_correction,
                gpu_matrix_preimage_residual, gpu_matrix_record_compiled_write,
                ...
                gpu_small_matrix_copy_preimage_status_async, gpu_small_matrix_create,
                ...
                gpu_small_matrix_wait_preimage_status,
            },
...
impl GpuMatrixFamilyDescriptorTable {
    ...
    pub fn bind_live_source_descriptors(
        ...
        let launch_stream = stream.clone();
...
impl PartialEq for GpuSmallMatrix {
    fn eq(&self, other: &Self) -> bool {
        self.params == other.params &&
            ...
            self.bound_domain == other.bound_domain &&
...
impl GpuSmallMatrix {
    ...
    pub fn output_descriptor(&self) -> Result<GpuSmallMatrixOutputDescriptor, SmallMatrixError> {
        GpuSmallMatrixOutputDescriptor::for_shape_in_domain(
            &self.params,
            self.rows,
            self.columns,
    ...
    pub fn binding_descriptor(
        ...
        Ok(GpuSmallMatrixBindingDescriptor {
            ...
            storage_columns: raw.storage_columns,
    ...
    pub fn centered_rebase_allocation_bytes(
        ...
        Self::allocation_bytes_for_shape_in_domain(
            destination,
            self.rows,
            self.columns,
            self.magnitude_bytes,
    ...
    pub fn centered_rebase(
        ...
        let output = Self::new_empty_in_domain(
            destination,
            self.rows,
            self.columns,
            self.max_coefficient_bound.clone(),
    ...
    pub fn slice_columns(&self, start: usize, end: usize) -> Self {
        ...
        let out = Self::new_empty_in_domain(
            &self.params,
            self.rows,
            end - start,
            self.max_coefficient_bound.clone(),
    ...
    pub(crate) fn new_empty_in_domain(
        params: &GpuDCRTPolyParams,
        rows: usize,
        columns: usize,
        max_coefficient_bound: BigUint,
        ...
        Self::new_empty_checked_in_domain(
            params,
            rows,
            columns,
            max_coefficient_bound,
...
impl SmallPolyMatrix for GpuSmallMatrix {
    ...
    fn from_canonical_coefficients_in_domain(
        ...
        let out =
            Self::new_empty_in_domain(params, rows, columns, max_coefficient_bound, bound_domain)?;
        let status =
...
impl PolyMatrixSmallRhs for GpuDCRTPolyMatrix {
    ...
    fn gadget_decompose_row_blocks(
        ...
        let bound: BigUint = (&base + BigUint::from(1u8)) >> 1usize;
        ...
        let out = GpuSmallMatrix::new_empty_in_domain(
            &source.params,
            rows,
            source.ncol,
            bound.clone(),
...
impl std::fmt::Debug for GpuP1CovarianceCache {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.debug_struct("GpuP1CovarianceCache").field("raw", &self.raw).finish()
    }
}
...
impl GpuDCRTPolyMatrix {
    /// The eager typed matrix reads its domain from its native owner. Raw Graph
    /// operations use the plan's PhysicalValue encoding instead.
    pub fn is_ntt(&self) -> bool {
        let mut format = -1;
        let status = unsafe { gpu_matrix_get_format(self.raw, &mut format) };
        check_status(status, "gpu_matrix_get_format");
        format == GPU_POLY_FORMAT_EVAL
    }
    /// Emit coefficient-domain gadget decomposition directly into an explicit
    /// graph. Local binding indices 0..L identify input limbs and L..2L
    /// identify output limbs, where L is the active CRT limb count.
    pub fn decompose_coeff_into_graph(
        &self,
        base_bits: u32,
        output: &Self,
        ...
        if self.params.ctx_raw() != output.params.ctx_raw() ||
            self.level != output.level ||
            self.is_ntt() ||
            output.is_ntt()
        {
            return Err(GpuNativeGraphError::Native(
                "coefficient graph decomposition requires matching coefficient matrices".into(),
            ));
        }
        let status = unsafe {
            gpu_matrix_decompose_coeff_graph(
                self.raw,
                base_bits,
                output.raw,
                dropped_moduli,
    ...
    pub(crate) fn new_empty_with_state(
        ...
        Self { owner, params: params.clone(), nrow, ncol, level, raw }
    }
    ...
    pub(crate) fn ensure_eval_domain(&self) -> Self {
        ...
        // The native owner is the typed eager format authority. The CUDA
        // entry point is a no-op for an already evaluated matrix.
    ...
    fn compact_bytes_batch_homogeneous(matrices: &[&Self]) -> Vec<Vec<u8>> {
        ...
        let formats = matrices
            ...
            .map(|matrix| {
                if matrix.is_ntt() {
                    GPU_POLY_FORMAT_EVAL as u8
                } else {
                    GPU_POLY_FORMAT_COEFF as u8
                }
            })
            .collect::<Vec<_>>();
    ...
    /// Allocate a physical matrix with zero coefficients in the requested
    /// transform domain. The returned owner keeps the native allocation and
    /// its initial upload alive until subsequent graph work is complete.
    pub fn zero_with_state(
        ...
        params
            .matrix_allocation_bytes(level, rows, columns, is_ntt)
            .map_err(GpuNativeGraphError::Native)?;
        Ok(Self::new_zero_with_state(params, rows, columns, level, is_ntt))
    }

    /// Overwrite this already allocated owner with a canonical compact
    /// matrix, preserving every physical address bound into a compiled graph.
    ///
    /// # Safety
    ...
    pub unsafe fn load_compact_bytes_in_place_after_completion(
        ...
        let target_evaluation = self.is_ntt();
        ...
        if target_evaluation && unsafe { gpu_matrix_ntt_all(self.raw) } != 0 {
            return Err(GpuNativeGraphError::Native(crate::poly::dcrt::gpu::last_error_string()));
        }
        Ok(())
    }
    ...
    fn new_zero_with_state(
        ...
        let mut out = Self::new_empty_with_state(params, nrow, ncol, level, is_ntt, None);
        if nrow == 0 || ncol == 0 {
            return out;
        }
        let bytes_per_poly =
    ...
    /// memory. No seed value is copied from the host.
    #[doc(hidden)]
    ...
    pub(crate) fn gauss_samp_gq_arb_base_into(
        ...
        assert!(
            ...
            "Gaussian body requires coefficient matrices"
        );
        let status = unsafe {
    ...
    pub(crate) fn mul_vertical_pair(top: &Self, bottom: &Self, rhs: &Self) -> Self {
        ...
        debug_assert!(
            top.is_ntt() && bottom.is_ntt() && rhs.is_ntt(),
            "vertical pair requires Eval"
        );
        let out = Self::new_empty_with_state(
            &top.params,
    ...
    pub fn from_rns_snapshot(
        ...
        if !snapshot.bytes.as_slice().is_empty() {
            let format =
                if snapshot.is_ntt() { GPU_POLY_FORMAT_EVAL } else { GPU_POLY_FORMAT_COEFF };
...
impl PolyMatrix for GpuDCRTPolyMatrix {
    ...
    fn multiply_polys_batch_out_of_place(inputs: Vec<(Arc<Self>, Self::P)>) -> Vec<Self> {
        ...
        for (index, (matrix, mut scalar)) in inputs.into_iter().enumerate() {
            let matrix =
                if matrix.is_ntt() { matrix } else { Arc::new(matrix.ensure_eval_domain()) };
            if !scalar.is_ntt() {
                scalar.ntt_in_place();
            }
            let key = (
    ...
    fn into_compact_bytes(self) -> Vec<u8> {
        let mut this = self;
        this.intt_all_in_place();
        let format = GPU_POLY_FORMAT_COEFF;

        let level = this.level;
        let coeff_count = this
            .nrow
            .saturating_mul(this.ncol)
            .saturating_mul(this.params.ring_dimension() as usize);
        let coeff_bits_upper = this
            .params
            .moduli()
            .iter()
            .take(level + 1)
    ...
    fn try_from_compact_bytes(
        ...
        if decoded.format != GPU_POLY_FORMAT_COEFF as u8 {
            return Err(crate::matrix::CompactMatrixDecodeError::InvalidHeader(
                "matrix artifact requires coefficient format",
            ));
        }
    ...
    fn try_from_cpu_staging_columns(
        ...
        if row_bytes != 0 {
            tile.as_mut_slice().par_chunks_mut(row_bytes).enumerate().for_each(|(row, target)| {
                let offset = (row * ncol + start) * bytes_per_poly;
    ...
    fn copy_to_params_direct(&self, params: &<Self::P as Poly>::Params) -> Option<Self> {
        ...
        let output = Self::new_empty_with_state(
            params,
            self.nrow,
            self.ncol,
            self.level,
    ...
    fn zero_compact_bytes(
        ...
        GpuDCRTPolyMatrix::zero_compact_bytes(params, nrow, ncol, level)
    }
...
mod tests {
    ...
    #[test]
    #[sequential]
    fn compact_upload_reuses_bound_matrix_address_and_rejects_mismatches() {
        ...
        let first = source(7);
        let second = source(19);
        let first_bytes = first.to_compact_bytes();
        let second_bytes = second.to_compact_bytes();
        let cpu_params = gpu_test_params();
        let mut coefficients = vec![BigUint::ZERO; cpu_params.ring_dimension() as usize];
        coefficients[0] = BigUint::from(7u8);
        let cpu_first = DCRTPolyMatrix::from_poly_vec(
            &cpu_params,
            vec![vec![DCRTPoly::from_biguints(&cpu_params, &coefficients)]],
        );
        assert_eq!(first_bytes, cpu_first.to_compact_bytes());
        let modulus = cpu_params.modulus();
        let half: BigUint = modulus.as_ref() >> 1usize;
        coefficients[1] = modulus.as_ref() - 1u8;
        coefficients[2] = half.clone();
        coefficients[3] = &half + 1u8;
        coefficients[4] = modulus.as_ref() - 3u8;
        let cpu_varied = DCRTPolyMatrix::from_poly_vec(
            &cpu_params,
            vec![vec![DCRTPoly::from_biguints(&cpu_params, &coefficients)]],
        );
        let gpu_varied = GpuDCRTPolyMatrix::from_poly_vec_row(
            &params,
            vec![GpuDCRTPoly::from_biguints(&params, &coefficients)],
        )
        .into_coeff_domain();
        let varied_bytes = cpu_varied.to_compact_bytes();
        assert_eq!(gpu_varied.to_compact_bytes(), varied_bytes);
        let owner = Arc::new(GpuDCRTPolyMatrix::zero_with_state(&params, 1, 1, false).unwrap());
        let shared = Arc::clone(&owner);
        let addresses = owner
            .binding_components()
            .unwrap()
            .iter()
            .map(|part| part.data_address)
            .collect::<Vec<_>>();
        unsafe {
            shared
                .load_compact_bytes_in_place_after_completion(
                    &first_bytes,
                    params.moduli(),
                    params.ring_dimension(),
                )
        ...
        assert_eq!(owner.to_compact_bytes(), first_bytes);
        unsafe {
            owner
                .load_compact_bytes_in_place_after_completion(
                    &second_bytes,
        ...
        assert_eq!(shared.to_compact_bytes(), second_bytes);
        unsafe {
            owner
                .load_compact_bytes_in_place_after_completion(
                    &varied_bytes,
        ...
        assert_eq!(shared.to_compact_bytes(), varied_bytes);
        assert_eq!(
            addresses,
            owner
                .binding_components()
        ...
        let mut wrong_basis = params.moduli().to_vec();
        wrong_basis[0] = wrong_basis[0].wrapping_add(2);
        assert!(
            unsafe {
        ...
        assert!(
            unsafe {
                owner.load_compact_bytes_in_place_after_completion(
                    &first_bytes,
                    params.moduli(),
                    params.ring_dimension() + 1,
                )
            }
            .is_err()
        );
        let wrong_shape = GpuDCRTPolyMatrix::from_poly_vec_row(
            &params,
            vec![
                GpuDCRTPoly::from_usize_to_constant(&params, 1),
                GpuDCRTPoly::from_usize_to_constant(&params, 2),
            ],
        )
        .into_coeff_domain()
        .to_compact_bytes();
        ...
        let eval_owner = GpuDCRTPolyMatrix::zero_with_state(&params, 1, 1, true).unwrap();
        unsafe {
            eval_owner.load_compact_bytes_in_place_after_completion(
                &first_bytes,
                params.moduli(),
                params.ring_dimension(),
            )
        ...
        .unwrap();
        assert!(eval_owner.is_ntt());
        assert_eq!(eval_owner.to_compact_bytes(), first_bytes);
    }

    #[test]
    #[sequential]
    fn binding_limbs_match_mixed_width_owner_allocation() {
        let ring_dimension = 32;
        let narrow = DCRTPolyParams::new(ring_dimension, 1, 28, 8, None, None).to_crt().0[0];
        let wide = DCRTPolyParams::new(ring_dimension, 1, 50, 8, None, None).to_crt().0[0];
        let params = GpuDCRTPolyParams::new(ring_dimension, vec![narrow, wide], 8, None);
        let matrix = GpuDCRTPolyMatrix::from_poly_vec(
            ...
            (0..2)
                .map(|row| {
                    (0..3)
                        .map(|column| {
                            GpuDCRTPoly::from_usize_to_constant(&params, row * 3 + column + 1)
                        })
                        .collect()
                })
                .collect(),
        ...
        let components = matrix.binding_components().unwrap();
        let limbs = matrix.binding_limbs().unwrap();
        assert_eq!(limbs.len(), 2);
        assert_eq!(limbs.iter().map(|limb| limb.coefficient_bytes).collect::<Vec<_>>(), vec![4, 8]);
        for limb in limbs.iter() {
            let component = &components[limb.component_index];
            assert_eq!(limb.data_address, component.data_address + limb.byte_offset as u64);
            assert_eq!(limb.poly_stride_bytes, component.bytes_per_poly);
            assert_eq!(limb.row_stride_bytes, 3 * component.bytes_per_poly);
            assert_eq!(limb.scratch_offset_bytes, component.bytes_per_poly / 2);
            assert_eq!(limb.modulus, params.moduli()[limb.crt_limb_index]);
            assert!(
                limb.byte_offset + ring_dimension as usize * limb.coefficient_bytes <=
                    limb.scratch_offset_bytes
            );
            for (row, column, value) in [(0usize, 0usize, 1u64), (1, 2, 6)] {
                let address = limb.data_address +
                    (row * limb.row_stride_bytes + column * limb.poly_stride_bytes) as u64;
                let mut bytes = vec![0; limb.coefficient_bytes];
                params.download_device_bytes(limb.physical_device, address, &mut bytes).unwrap();
                let observed = bytes
                    .into_iter()
                    .enumerate()
                    .fold(0u64, |word, (shift, byte)| word | (u64::from(byte) << (8 * shift)));
                assert_eq!(observed, value % limb.modulus);
            }
        }
    }
    ...
    fn test_gpu_matrix_zero_compact_bytes_roundtrip() {
        ...
        let params = gpu_test_params();
        let gpu_params = gpu_params_from_cpu(&params);
        let cases = [(2usize, 3usize, 0usize), (1usize, 4usize, 1usize)];

        for (nrow, ncol, level) in cases {
            let bytes = GpuDCRTPolyMatrix::zero_compact_bytes(&gpu_params, nrow, ncol, level);
            let decoded = GpuDCRTPolyMatrix::from_compact_bytes(&gpu_params, &bytes);
            let expected =
                GpuDCRTPolyMatrix::new_zero_with_state(&gpu_params, nrow, ncol, level, false);
            assert_eq!(
                decoded, expected,
    ...
    #[test]
    #[sequential]
    fn balanced_compact_small_gadget_preserves_two_crt_towers() {
        gpu_device_sync();
        let cpu_params = DCRTPolyParams::new(8, 2, 17, 3, None, None);
        let gpu_params = gpu_params_from_cpu(&cpu_params);
        let moduli = cpu_params.moduli();
        assert!(moduli.len() >= 2);
        let modulus = cpu_params.modulus();
        let coefficients = vec![
            BigUint::from(0u8),
            BigUint::from(moduli[0] / 2),
            BigUint::from(moduli[0] / 2 + 1),
            BigUint::from(moduli[1] - 1),
            modulus.as_ref() / 2u8,
            modulus.as_ref() - 1u8,
            BigUint::from(moduli[0] + moduli[1]),
            BigUint::from(5u8),
        ];
        let cpu_source = DCRTPolyMatrix::from_poly_vec_row(
            &cpu_params,
        ...
        let gpu_source = GpuDCRTPolyMatrix::from_poly_vec_row(
            &gpu_params,
            vec![GpuDCRTPoly::from_biguints(&gpu_params, &coefficients)],
        );
        let cpu_digits = cpu_source.clone().gadget_decompose(true, None).unwrap();
        let gpu_digits = gpu_source.clone().gadget_decompose(true, None).unwrap();
        assert_eq!(gpu_digits.bound_domain(), CoefficientBoundDomain::PerCrtLimb);
        assert_eq!(gpu_digits.bound(), cpu_digits.max_coefficient_bound());
        let gpu_payload = gpu_digits.to_canonical_coefficients().unwrap();
        let cpu_payload = cpu_digits.to_canonical_coefficients().unwrap();
        assert_eq!(gpu_payload, cpu_payload);
        let cell_width = 1 + usize::try_from(gpu_digits.bound().bits().div_ceil(8)).unwrap();
        assert!(
            gpu_payload
                .chunks_exact(cell_width)
                .all(|cell| { BigUint::from_bytes_le(&cell[1..]) <= *gpu_digits.bound() })
        );
        assert!(gpu_payload.chunks_exact(cell_width * moduli.len()).any(|coefficient| {
            coefficient[..cell_width] != coefficient[cell_width..2 * cell_width]
        }));
        ...
        let schema = mxx_ir_core::artifact::ConcreteBoundedMatrixSchema {
            matrix: mxx_ir_core::types::ConcreteMatrixType {
                ring,
                rows: gpu_digits.rows_count(),
                columns: 1,
            },
            max_coefficient_bound: num_bigint::BigInt::from(gpu_digits.bound().clone()),
            bound_domain: CoefficientBoundDomain::PerCrtLimb,
        };
        let cpu_smr2 = crate::backend::poly::encode_small_matrix_artifact(
            &schema,
            &cpu_payload,
            mxx_ir_core::artifact::SmallMatrixSemanticKind::Generic,
        )
        .unwrap();
        let gpu_smr2 = crate::backend::poly::encode_small_matrix_artifact(
            &schema,
            &gpu_payload,
            mxx_ir_core::artifact::SmallMatrixSemanticKind::Generic,
        )
        .unwrap();
        assert_eq!(gpu_smr2, cpu_smr2);
        assert_eq!(&gpu_smr2[..4], b"SMR2");
        assert_eq!(gpu_smr2[5], 1);
        let decoded = GpuSmallMatrix::from_canonical_coefficients_in_domain(
            &gpu_params,
            gpu_digits.rows_count(),
            1,
            gpu_digits.bound().clone(),
            CoefficientBoundDomain::PerCrtLimb,
            &gpu_payload,
        )
        .unwrap();
        assert_eq!(decoded.to_canonical_coefficients().unwrap(), gpu_payload);
        let gadget = GpuDCRTPolyMatrix::small_gadget_matrix(&gpu_params, 1);
        let reconstructed = gadget.multiply_small_rhs(&decoded).unwrap();
        assert_eq!(reconstructed, gpu_source);
    }
