...
pub struct RealPolyContext<P: Poly> {
    ...
    pub real_to_l1_luts: Vec<PackedPlt<P>>,
}
...
impl<P: Poly> RealPolyContext<P> {
    pub fn setup(
        ...
        let mut real_to_l1_luts = Vec::with_capacity(l1_moduli.len());
        for &modulus in l1_moduli.iter() {
            ...
            let real_to_l1_map_slot: HashMap<BigUint, (usize, BigUint)> =
                HashMap::from_iter((0..=max_real as usize).map(|t| {
                    let input = BigUint::from(t as u64);
                    let output = BigUint::from(t as u64 / scale);
                    (input, (t as usize, output))
                }));
            real_to_l1_luts.push(PackedPlt::setup(
                circuit,
                params,
                max_degree,
                real_to_l1_map_slot,
        ...
        Self { scale, l1_ctx: Arc::new(l1_ctx), l1_to_real_luts, real_to_l1_luts }
    }
}
...
impl<P: Poly> RealPoly<P> {
    ...
    pub fn add(&self, other: &Self, circuit: &mut PolyCircuit<P>) -> Self {
        debug_assert_eq!(self.ctx.as_ref(), other.ctx.as_ref());
        let mut new_inner = Vec::with_capacity(self.inner.len());
        for (&a, &b) in self.inner.iter().zip(other.inner.iter()) {
            let sum = circuit.add_gate(a, b);
            new_inner.push(sum);
        }
        Self { ctx: self.ctx.clone(), inner: new_inner }
    }

    pub fn to_l1_poly(&self, circuit: &mut PolyCircuit<P>) -> L1Poly<P> {
        let mut new_inner = Vec::with_capacity(self.inner.len());
        for (lut, &input) in self.ctx.real_to_l1_luts.iter().zip(self.inner.iter()) {
            let output = lut.lookup_all(circuit, input);
            new_inner.push(output);
        }
