...
// Explicit-graph variant: each destination limb is a scalar argument, so
// replay patches the real matrix owners without a transient device pointer table.
__global__ void matrix_decompose_one_limb_kernel(
    const uint8_t *src_base, uint8_t *dst_base,
    size_t src_stride_bytes, size_t dst_stride_bytes,
    uint8_t src_coeff_bytes, uint8_t dst_coeff_bytes,
    size_t poly_count, size_t n, size_t src_cols, size_t out_cols,
    size_t log_base_q, uint32_t src_bits, uint64_t src_modulus,
    uint64_t dst_modulus, uint32_t base_bits, uint32_t digits_per_tower,
    bool balanced, size_t src_digit_offset_base,
    size_t poly_offset, size_t digit_offset)
{
    const size_t coefficient = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    const size_t poly = poly_offset + static_cast<size_t>(blockIdx.y);
    const size_t digit_index = digit_offset + static_cast<size_t>(blockIdx.z);
    if (coefficient >= n || poly >= poly_count || digit_index >= digits_per_tower) return;
    const uint64_t residue = matrix_load_limb_u64(
        src_base, poly, coefficient, src_stride_bytes, src_coeff_bytes);
    uint64_t digit = 0;
    if (balanced)
    {
        int64_t value = centered_lift_u64(residue, src_modulus);
        ...
        digit = signed_digit_to_residue(signed_digit, dst_modulus);
    }
    else
    {
        const uint32_t shift = static_cast<uint32_t>(digit_index) * base_bits;
        if (shift < src_bits && shift < 64)
        {
            const uint32_t width = min(base_bits, src_bits - shift);
            const uint64_t mask = width >= 64 ? ~uint64_t{0} : ((uint64_t{1} << width) - 1);
            digit = (residue >> shift) & mask;
        }
        if (dst_modulus != 0 && digit >= dst_modulus) digit %= dst_modulus;
    }
    const size_t row = poly / src_cols;
    const size_t column = poly - row * src_cols;
    const size_t out_row = row * log_base_q + src_digit_offset_base + digit_index;
    matrix_store_limb_u64(dst_base, out_row * out_cols + column,
        coefficient, dst_stride_bytes, dst_coeff_bytes, digit);
}
...
extern "C" int gpu_matrix_decompose_coeff_graph(
    const GpuMatrix *src, uint32_t base_bits, GpuMatrix *out,
    size_t dropped_moduli, void *launch_stream)
{
    if (!src || !out || !src->ctx || src->ctx != out->ctx ||
        src->level != out->level || src->level < 0 ||
        base_bits == 0 || base_bits >= 63 || !launch_stream)
        return set_error("invalid coefficient graph decomposition arguments");
    const size_t limb_count = static_cast<size_t>(src->level + 1);
    if (dropped_moduli >= limb_count ||
        src->ctx->moduli.size() < limb_count ||
        src->ctx->limb_gpu_ids.size() < limb_count)
        return set_error("invalid coefficient graph decomposition CRT basis");
    uint32_t crt_bits = 0;
    for (size_t limb = 0; limb < limb_count; ++limb)
        crt_bits = std::max(crt_bits, bit_width_u64(src->ctx->moduli[limb]));
    const uint32_t digits_per_tower = (crt_bits + base_bits - 1) / base_bits;
    const size_t source_limbs = limb_count - dropped_moduli;
    if (!digits_per_tower || src->rows == 0 || src->cols == 0 ||
        src->rows > std::numeric_limits<size_t>::max() /
            (static_cast<size_t>(digits_per_tower) * source_limbs) ||
        out->rows != src->rows * static_cast<size_t>(digits_per_tower) * source_limbs ||
        out->cols != src->cols)
        return set_error("coefficient graph decomposition shape mismatch");
    const size_t poly_count = src->rows * src->cols;
    const size_t n = static_cast<size_t>(src->ctx->N);
    auto *stream = reinterpret_cast<cudaStream_t>(launch_stream);
    for (size_t source = 0; source < source_limbs; ++source)
    {
        const dim3 source_id = src->ctx->limb_gpu_ids[source];
        int device = -1;
        if (matrix_limb_device(src, source_id, &device) != 0) return 1;
        if (cudaSetDevice(device) != cudaSuccess) return set_error(cudaGetLastError());
        if (!mxx_gpu_graph_builder_for_stream(src->ctx, launch_stream))
            return set_error("coefficient decomposition requires an explicit graph operation");
        const uint8_t *source_base = matrix_limb_ptr_by_id(src, 0, source_id);
        size_t source_stride = 0;
        uint8_t source_width = 0;
        if (!source_base || !matrix_limb_metadata_by_id(
            src, source_id, &source_stride, &source_width)) return 1;
        for (size_t destination = 0; destination < limb_count; ++destination)
        {
            const dim3 destination_id = src->ctx->limb_gpu_ids[destination];
            int destination_device = -1;
            if (matrix_limb_device(out, destination_id, &destination_device) != 0) return 1;
            if (destination_device != device)
                return set_error("graph decomposition spans multiple physical GPUs");
            uint8_t *destination_base = matrix_limb_ptr_by_id(out, 0, destination_id);
            size_t destination_stride = 0;
            uint8_t destination_width = 0;
            if (!destination_base || !matrix_limb_metadata_by_id(
                out, destination_id, &destination_stride, &destination_width)) return 1;
            for (size_t poly_offset = 0; poly_offset < poly_count;
                 poly_offset += kDecomposeMaxGridY)
            {
                const size_t poly_chunk = std::min(kDecomposeMaxGridY, poly_count - poly_offset);
                for (size_t digit_offset = 0; digit_offset < digits_per_tower;
                     digit_offset += kDecomposeMaxGridZ)
                {
                    const size_t digit_chunk = std::min(
                        kDecomposeMaxGridZ, static_cast<size_t>(digits_per_tower) - digit_offset);
                    const dim3 grid(
                        static_cast<uint32_t>((n + kDecomposeThreads - 1) / kDecomposeThreads),
                        static_cast<uint32_t>(poly_chunk), static_cast<uint32_t>(digit_chunk));
                    const MxxGraphPatch patches[] = {
                        decompose_pointer_patch(0, 0, static_cast<uint32_t>(source)),
                        decompose_pointer_patch(1, 0,
                            static_cast<uint32_t>(limb_count + destination)),
                    };
                    const int status = mxx_gpu_launch_kernel(src->ctx, stream,
                        matrix_decompose_one_limb_kernel, grid, dim3(kDecomposeThreads), 0,
                        patches, std::size(patches), source_base, destination_base,
                        source_stride, destination_stride, source_width, destination_width,
                        poly_count, n, src->cols, out->cols,
                        static_cast<size_t>(digits_per_tower) * source_limbs,
                        bit_width_u64(src->ctx->moduli[source]), src->ctx->moduli[source],
                        src->ctx->moduli[destination], base_bits, digits_per_tower,
                        true, source * static_cast<size_t>(digits_per_tower),
                        poly_offset, digit_offset);
                    if (status != 0) return status;
                }
            }
        }
