...
Use an explicit version tag or digest for reproducible runs; retain a user-specified tag when required by the task. The `latest` tag is mutable and does not identify the source build.
