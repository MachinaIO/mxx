...
namespace LeftAuthority70806
def owner : Owner := ⟨.program ⟨214⟩, ⟨2348⟩⟩
def authority : Authority := .operator
def bound : CoeffClass := .finite ⟨1, by decide⟩
def producerEvent : Nat := 70806
def resultEvent : Nat := 70807
def frameStart : Nat := 70744
theorem leaf : AuthorityLeafAt history producerEvent resultEvent frameStart owner authority bound := by
  exact .resultCoefficient (by decide) (by rfl) (by rfl)
def actual (selector : Nat)
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70806

namespace LeftAuthority70809
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70809

namespace LeftAuthority70815
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70815

namespace LeftAuthority70842
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70842

namespace LeftAuthority70850
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70850

namespace LeftAuthority70887
...
theorem leaf : AuthorityLeafAt history producerEvent resultEvent frameStart owner authority bound := by
  exact .resultExact (terms := [{ coefficient := (1), monomial := { centralFactors := [], orderedFactors := [⟨.program ⟨214⟩, ⟨21252⟩⟩] } }]) (recordedCoefficientBound := .finite 136065468) (summary := .exactZero) (summaryProducer := none) (by decide) (by rfl) (by rfl) (by simp [bound, RecordedBoundRefines])
def actual (selector : Nat)
    (witness : Witness document history (some selector) 100418593683253592432016548326729029359133068138294319235841) : Nat :=
  witness.authorityMagnitude resultEvent
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70887

namespace LeftAuthority70989
def owner : Owner := ⟨.program ⟨214⟩, ⟨15817⟩⟩
def authority : Authority := .programFamilyFact
def bound : CoeffClass := .finite ⟨16, by decide⟩
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority70989

namespace LeftAuthority71000
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71000

namespace LeftAuthority71003
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71003

namespace LeftAuthority71005
def owner : Owner := ⟨.program ⟨214⟩, ⟨110⟩⟩
def authority : Authority := .operator
def bound : CoeffClass := .exactZero
def producerEvent : Nat := 71005
def resultEvent : Nat := 71006
def frameStart : Nat := 70953
theorem leaf : AuthorityLeafAt history producerEvent resultEvent frameStart owner authority bound := by
  exact .resultCoefficient (by decide) (by rfl) (by rfl)
def actual (selector : Nat)
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71005

namespace LeftAuthority71014
def owner : Owner := ⟨.program ⟨214⟩, ⟨6544⟩⟩
def authority : Authority := .factStore
def bound : CoeffClass := .large
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71014

namespace LeftAuthority71022
def owner : Owner := ⟨.program ⟨214⟩, ⟨6696⟩⟩
def authority : Authority := .operator
def bound : CoeffClass := .large
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71022

namespace LeftAuthority71037
...
theorem derived (selector : Nat)
    ...
    BoundDerivedAt history producerEvent frameStart owner (.authority authority) bound
      (actual selector witness) := by
  exact .authority witness.toAuthorityWitness leaf
end LeftAuthority71037

namespace LeftAuthority71045
