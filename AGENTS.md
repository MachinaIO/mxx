...
## Lean correctness work

- The shared Lean package lives under `lean/Mxx`. Each owning Rust crate keeps machine-generated
  modules under its `lean/<Module>/Generated/` directory and human- or agent-written proofs under
  its `lean/<Module>/Proofs/` directory.
- Proofs may use any kernel-checked tactic. `native_decide` is allowed only for checker-evaluation
  facts and must be reported by theorem verification because it enlarges the trusted base.
- Do not declare Lean axioms. Reviewed sampler facts belong in `MxxBoundedSamplerContract` as
