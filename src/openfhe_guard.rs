...
static GLOBAL_WARMED: OnceLock<()> = OnceLock::new();
...
pub(crate) fn ensure_openfhe_global_warmup() {
    GLOBAL_WARMED.get_or_init(|| {
        // Touch OpenFHE's sampling path once.
        let _ = ffi::GenerateIntegerKarney(0.0, 1.0);
    });
}
