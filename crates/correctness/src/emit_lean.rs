...
pub struct EmittedProtocol {
    ...
    pub(crate) operational_raw_ir_template: String,
    pub(crate) operational_documents: Vec<OperationalBinaryDocument>,
    pub(crate) operational_decoder_targets: Vec<EmittedOperationalDecoderTarget>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct OperationalBinaryDocument {
    pub name: String,
    pub bytes: Vec<u8>,
    decode_kind: OperationalBinaryDocumentKind,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum OperationalBinaryDocumentKind {
    Program,
    Derivation,
}
...
pub fn emit_protocol_for(
    ...
    let operational_documents = operational_binary_documents(protocol, &lean_name)?;
    ...
    let operational_raw_ir_template =
        lean_raw_operational_source(&lean_name, protocol, module_root, &operational_documents)?;
    ...
    let operational_decoder_targets = protocol
        ...
        .map(|target| {
            ...
            let residual_type = match &residual_stage
                .graph
                .root_scope()
                .node(residual.value.node)
                .expect("validated operational target residual node")
                .output_types()[residual.value.port.0 as usize]
            {
                WireType::Matrix(matrix_type) => matrix_type.clone(),
                WireType::IndexedFamily { element, .. } => match element.as_ref() {
                    WireType::Matrix(matrix_type) => matrix_type.clone(),
                    _ => unreachable!("validated operational target residual matrix family type"),
                },
                _ => unreachable!("validated operational target residual matrix type"),
            };
    ...
    Ok(EmittedProtocol {
        ...
        proof_ir,
        derivation_ir,
        operational_raw_ir_template,
        operational_documents,
        operational_decoder_targets,
    })
}

fn operational_binary_documents(
    protocol: &ProtocolDecl,
    lean_name: &str,
) -> Result<Vec<OperationalBinaryDocument>, EmitError> {
    let mut documents = Vec::new();
    let mut push =
        |name: String, bytes: Result<Vec<u8>, crate::ir_binary::BinaryEncodeError>, decode_kind| {
            documents.push(OperationalBinaryDocument {
                name,
                bytes: bytes.map_err(|error| EmitError::BinaryTransport(error.to_string()))?,
                decode_kind,
            });
            Ok::<_, EmitError>(())
        };
    for stage in topological_stages(protocol) {
        let name = format!("{lean_name}_stage_{}", lower_identifier(&stage.id.0));
        push(
            name.clone(),
            crate::ir_binary::encode_prog(&stage.graph),
            OperationalBinaryDocumentKind::Program,
        )?;
        push(
            format!("{name}_derivation"),
            crate::ir_binary::encode_program_derivation(
                &stage.graph,
                Some(&stage.derivation_attachments),
            ),
            OperationalBinaryDocumentKind::Derivation,
        )?;
    }
    push(
        format!("{lean_name}_ideal"),
        crate::ir_binary::encode_prog(&protocol.bundle.ideal.graph),
        OperationalBinaryDocumentKind::Program,
    )?;
    push(
        format!("{lean_name}_ideal_derivation"),
        crate::ir_binary::encode_program_derivation(&protocol.bundle.ideal.graph, None),
        OperationalBinaryDocumentKind::Derivation,
    )?;
    for (index, requirement) in protocol.bundle.requirements.iter().enumerate() {
        ...
        push(
            name.clone(),
            crate::ir_binary::encode_prog(&requirement.graph),
            OperationalBinaryDocumentKind::Program,
        )?;
        push(
            format!("{name}_derivation"),
            crate::ir_binary::encode_program_derivation(&requirement.graph, None),
            OperationalBinaryDocumentKind::Derivation,
        )?;
    }
    if let Some(program) = protocol.bundle.comparator.program() {
        ...
        push(
            name.clone(),
            crate::ir_binary::encode_prog(&program.graph),
            OperationalBinaryDocumentKind::Program,
        )?;
        push(
            format!("{name}_derivation"),
            crate::ir_binary::encode_program_derivation(&program.graph, None),
            OperationalBinaryDocumentKind::Derivation,
        )?;
    }
    Ok(documents)
...
fn raw_document_token(name: &str) -> String {
    format!("__MXX_OPERATIONAL_DOCUMENT_{name}__")
}

fn lean_raw_operational_source(
    lean_name: &str,
    protocol: &ProtocolDecl,
    module_root: &str,
    documents: &[OperationalBinaryDocument],
) -> Result<String, EmitError> {
    let stages = topological_stages(protocol);
    let document_by_name = |name: &str| {
        documents.iter().find(|document| document.name == name).expect("complete raw document set")
    };
    let document_definitions = documents
        .iter()
        .map(|document| {
            let token = raw_document_token(&document.name);
            let decoder = match document.decode_kind {
                OperationalBinaryDocumentKind::Program => "Mxx.Ir.decodeProg",
                OperationalBinaryDocumentKind::Derivation => "Mxx.Ir.decodeProgramDerivation",
            };
            let force = match document.decode_kind {
                OperationalBinaryDocumentKind::Program => "Mxx.Certificate.forceProgOperationalDocument",
                OperationalBinaryDocumentKind::Derivation =>
                    "Mxx.Certificate.forceProgramDerivationOperationalDocument",
            };
            let result_type = match document.decode_kind {
                OperationalBinaryDocumentKind::Program => "Mxx.Ir.Prog",
                OperationalBinaryDocumentKind::Derivation => "Mxx.Certificate.ProgramDerivation",
            };
            let source = format!(
                "private def {name}Result : IO (Except Mxx.Ir.DecodeError {result_type}) := do\n  \
                 emitOperationalDocumentProgress {label} \"read\" \"start\" none\n  \
                 let readStarted ← IO.monoNanosNow\n  \
                 let bytes ← IO.FS.readBinFile \"{token}\"\n  \
                 let readFinished ← IO.monoNanosNow\n  \
                 emitOperationalDocumentProgress {label} \"read\" \"complete\" (some (\"bytes=\" ++ toString bytes.size ++ \"; elapsed_ns=\" ++ toString (readFinished - readStarted)))\n  \
                 emitOperationalDocumentProgress {label} \"decode\" \"start\" none\n  \
                 let decodeStarted ← IO.monoNanosNow\n  \
                 let result := {decoder} bytes\n  \
                 let decodeFinished ← IO.monoNanosNow\n  \
                 emitOperationalDocumentProgress {label} \"decode\" \"complete\" (some (\"elapsed_ns=\" ++ toString (decodeFinished - decodeStarted)))\n  \
                 pure result",
                name = document.name,
                label = lean_string(&document.name),
            );
            source.replace(
                &format!("let result := {decoder} bytes\n  let decodeFinished", decoder = decoder),
                &format!(
                    "let result := {decoder} bytes\n  let forced ← match result with\n    | .ok value => some <$> {force} {label} value\n    | .error _ => pure none\n  let decodeFinished",
                    decoder = decoder,
                    force = force,
                    label = lean_string(&document.name),
                ),
            ).replace(
                "toString (decodeFinished - decodeStarted)))\n  pure result",
                "toString (decodeFinished - decodeStarted) ++ match forced with\n    | none => \"\"\n    | some stats => \"; forced_entries=\" ++ toString stats.entries ++ \"; checksum=\" ++ toString stats.checksum))\n  pure result",
            )
        })
        .collect::<Vec<_>>()
        .join("\n\n");
    let document_result_lets = documents
        .iter()
        .map(|document| {
            let force = match document.decode_kind {
                OperationalBinaryDocumentKind::Program =>
                    "Mxx.Certificate.forceProgOperationalDocument",
                OperationalBinaryDocumentKind::Derivation =>
                    "Mxx.Certificate.forceProgramDerivationOperationalDocument",
            };
            format!(
                "  let {name}Decoded ← {name}Result\n  match {name}Decoded with\n  | .ok value => let _ ← {force} {label} value; pure ()\n  | .error _ => pure ()",
                name = document.name,
                force = force,
                label = lean_string(&document.name),
            )
    ...
    let mut derivation_entries = Vec::new();
    for stage in &stages {
        let program_name = format!("{lean_name}_stage_{}", lower_identifier(&stage.id.0));
        let derivation_name = format!("{program_name}_derivation");
        let _ = document_by_name(&program_name);
        let _ = document_by_name(&derivation_name);
        program_lets.push(format!("    let {program_name} ← {program_name}Decoded"));
        derivation_lets.push(format!("    let {derivation_name} ← {derivation_name}Decoded"));
        derivation_entries.push(format!("({}, {derivation_name})", lean_string(&stage.id.0)));
    }
    let ideal_name = format!("{lean_name}_ideal");
    let ideal_derivation_name = format!("{ideal_name}_derivation");
    let _ = document_by_name(&ideal_name);
    let _ = document_by_name(&ideal_derivation_name);
    program_lets.push(format!("    let {ideal_name} ← {ideal_name}Decoded"));
    derivation_lets
        .push(format!("    let {ideal_derivation_name} ← {ideal_derivation_name}Decoded"));
    derivation_entries.push(format!("(\"ideal\", {ideal_derivation_name})"));
    for (index, requirement) in protocol.bundle.requirements.iter().enumerate() {
        let name = format!("{lean_name}_requirement_{index}");
        let derivation_name = format!("{name}_derivation");
        let _ = requirement;
        let _ = document_by_name(&name);
        let _ = document_by_name(&derivation_name);
        program_lets.push(format!("    let {name} ← {name}Decoded"));
        derivation_lets.push(format!("    let {derivation_name} ← {derivation_name}Decoded"));
        derivation_entries.push(format!("(\"requirement-{index}\", {derivation_name})"));
    }
    let comparator_program = protocol.bundle.comparator.program().map(|_| {
        let name = format!("{lean_name}_comparator");
        let derivation_name = format!("{name}_derivation");
        let _ = document_by_name(&name);
        let _ = document_by_name(&derivation_name);
    ...
    let parameters = protocol
        ...
        .join(", ");
    Ok(format!(
        "-- Generated by {GENERATOR_VERSION}; do not edit.\nimport Mxx.Ir.BinaryFormat\nimport Mxx.Certificate.Workflow\n\nnamespace {module_root}.Generated.{lean_name}\n\nprivate def emitOperationalDocumentProgress\n    (document phase event : String) (detail : Option String) : IO Unit :=\n  let detailField := match detail with\n    | none => \"\"\n    | some value => \" detail=\" ++ value\n  IO.eprintln (\"operational_progress phase=document_\" ++ phase ++ \" event=\" ++ event ++\n    \" document=\" ++ document ++ detailField)\n\n{document_definitions}\n\ndef {lean_name}_decodedFromRawFiles : IO (Except Mxx.Ir.DecodeError (Mxx.Certificate.ClosedProtocolDecl × List (String × Mxx.Certificate.ProgramDerivation))) := do\n{document_result_lets}\n  pure do\n{program_lets}\n{derivation_lets}\n    let protocol : Mxx.Certificate.ClosedProtocolDecl := {{ parameters := [{parameters}], bundle := {bundle} }}\n    pure (protocol, [{derivation_entries}])\n\nend {module_root}.Generated.{lean_name}\n",
        program_lets = program_lets.join("\n"),
        derivation_lets = derivation_lets.join("\n"),
        derivation_entries = derivation_entries.join(", "),
    ))
}
...
mod tests {
    ...
    fn v11_transport_is_byte_stable_and_hashes_closed_decoder_targets() {
        ...
        assert_eq!(first.operational_raw_ir_template, second.operational_raw_ir_template);
        assert_eq!(first.operational_documents, second.operational_documents);
        ...
        assert!(first.operational_raw_ir_template.contains("IO.FS.readBinFile"));
        assert!(first.operational_raw_ir_template.contains("ToyExample_decodedFromRawFiles"));
        assert!(!first.operational_raw_ir_template.contains("decodeHexChunks"));
        assert!(!first.operational_documents.is_empty());
