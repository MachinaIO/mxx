...
pub enum LayoutError {
    ...
    #[error("matrix ordered CRT basis does not match the supplied CRT context")]
    MatrixCrtBasisMismatch,
...
impl LeanRingLayout {
    ...
    pub fn validate_request(
        ...
        if matrix.ring.modulus() != self.modulus {
            return Err(LayoutError::MatrixModulusMismatch);
        }
        if matrix.ring.ring_dimension() != self.ring_dimension {
            return Err(LayoutError::MatrixRingDimensionMismatch);
        }
        if matrix.ring.crt_moduli() != self.crt_moduli.as_slice() {
            return Err(LayoutError::MatrixCrtBasisMismatch);
        }
...
mod tests {
    ...
    fn validates_concrete_ir_request_for_each_mode() {
        let context = layout(vec![17, 97], 8, 4, 4).validate().unwrap();
        ...
        reordered.crt_moduli.reverse();
        assert_eq!(
            reordered.validate_request(&matrix, &16.into(), 4, LeanGadgetMode::Regular),
            Err(LayoutError::MatrixCrtBasisMismatch)
        );
    }
}
