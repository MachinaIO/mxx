...
use crate::{element::PolyElem, poly::PolyParams};
use dashmap::DashMap;
...
impl<P: Poly> PolyCircuit<P> {
    ...
    pub fn apply_lt_isolate(&mut self, input: GateId, lookup_id: usize) -> GateId {
        self.public_lookup_gate(input, lookup_id)
    }
    ...
    pub fn register_lt_isolate_lookup(&mut self, params: &P::Params, max_value: usize) -> usize {
        ...
        self.register_public_lookup(lt_isolate_lut)
    }
    ...
    pub fn register_general_lt_isolate_lookup(
        &mut self,
        params: &P::Params,
        norm_bound: usize,
    ) -> usize {
    ...
    pub fn isolate_coeffs(
        ...
        for i in 0..=degree_k {
            ...
            let rotated = if shift == 0 { input } else { self.rotate_gate(input, shift) };
            let isolated_coeff = self.apply_lt_isolate(rotated, lt_isolate_lookup_id);
...
mod tests {
    ...
    fn test_apply_lt_isolate() {
        let params = DCRTPolyParams::default();
        let mut circuit = PolyCircuit::new();
        let lt_isolate_id = circuit.register_lt_isolate_lookup(&params, 10);
        let inputs = circuit.input(1);
        let isolated = circuit.apply_lt_isolate(inputs[0], lt_isolate_id);
