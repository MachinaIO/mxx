...
use workspace_deps::{
    bench_estimator::{
        ...
        NaiveBGGVecBenchEstimator, PublicLutSampleAuxBenchEstimator,
    },
    bgg::{
...
async fn build_naive_vec_encoding_bench_estimator(
    ...
    let public_keys =
        public_key_sampler.sample(params, b"test_gpu_diamond_io_encoding_bench", &[true]);
    ...
    let (public_lut_aux_writer, lookup_base_matrix) = build_lwe_pubkey_vec_plt_evaluator(
        params,
        DEFAULT_HASH_KEY,
        cfg.d_secret,
        bench_dir.clone(),
    ...
    wait_for_all_writes(bench_dir.clone())
        .await
        .expect("DiamondIO encoding public lookup benchmark aux writes must flush");
