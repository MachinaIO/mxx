...
pub async fn flush_all_batches() -> io::Result<()> {
    let channel = BATCH_CHANNEL.get().ok_or_else(|| {
        io::Error::new(io::ErrorKind::NotFound, "Batch channel not initialized")
    })?;
...
pub async fn flush_all_batches_combined(base_dir: PathBuf) -> io::Result<String> {
                    ...
                    if let Some(lookup_id) = filename_str.strip_suffix("_batch_001.matrices") {
                        ...
                        let matrices: Vec<(usize, Vec<u8>)> = 
                            bincode::decode_from_slice(&data, bincode::config::standard())
                                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, format!("bincode: {e}")))?
                                .0;
    ...
    let mut header_size = 8; // number of lookups
    let mut data_sections = Vec::new();
    
    for (lookup_id, matrices) in &all_lookups {
        header_size += 8; // id length
        header_size += lookup_id.len(); // id bytes
        header_size += 24; // num_matrices + data_offset + data_size
