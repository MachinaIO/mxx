...
use crate::backend::{Backend, RuntimeValue};
use mxx_ir_core::{
...
///
/// This is deliberately generic over the backend and returns the real
/// `RuntimeValue`: matrices, compact values, families, blobs and trapdoors
...
pub fn clone_typed_runtime_input<B: Backend>(
    inputs: &BTreeMap<String, RuntimeValue<B>>,
    name: &str,
    expected: &mxx_ir_core::types::ConcreteWireType,
) -> Result<RuntimeValue<B>, RuntimeValueAccessError> {
    let value = inputs
        .get(name)
...
/// only the public Arc is cloned for the output value.
pub fn project_trapdoor_public<B: Backend>(
    value: &RuntimeValue<B>,
) -> Result<Arc<B::Matrix>, RuntimeValueAccessError> {
    match value {
        RuntimeValue::Trapdoor { public, .. } => Ok(public.clone()),
...
pub fn measure_typed_runtime_input<B: Backend>(
    inputs: &BTreeMap<String, RuntimeValue<B>>,
    name: &str,
    expected: &mxx_ir_core::types::ConcreteWireType,
    repetitions: usize,
...
pub fn measure_trapdoor_public<B: Backend>(
    value: &RuntimeValue<B>,
    repetitions: usize,
) -> Result<f64, RuntimeValueAccessError> {
    if repetitions == 0 {
...
pub fn dispatch_extract_coefficient<B: crate::Backend>(
    backend: &mut B,
    value: &B::Matrix,
    position: usize,
) -> Result<BigInt, B::Error> {
    backend.extract_coefficient(value, position)
}

pub fn dispatch_threshold_decode<B: crate::Backend>(
    backend: &mut B,
    value: &B::Matrix,
    plaintext_modulus: &BigInt,
    length: usize,
) -> Result<Vec<BigInt>, B::Error> {
    backend.threshold_decode(value, plaintext_modulus, length)
}

pub fn dispatch_pack_polynomial_coefficients<B: crate::Backend>(
    backend: &mut B,
    ty: &ConcreteMatrixType,
    bits: &[bool],
    coefficient_bits: usize,
) -> Result<B::Matrix, B::Error> {
    backend.pack_polynomial_coefficients(ty, bits, coefficient_bits)
}

pub fn dispatch_polynomial_from_values<B: crate::Backend>(
    backend: &mut B,
    ty: &ConcreteMatrixType,
    values: &[BigInt],
    evaluation: bool,
...
pub fn dispatch_polynomial_values<B: crate::Backend>(
    backend: &mut B,
    value: &B::Matrix,
    evaluation: bool,
) -> Result<Vec<BigInt>, B::Error> {
...
///
/// This is intentionally independent from `RuntimeValue<B>` and the backend:
...
pub fn measure_runtime_container_primitive<B: Backend>(
    node: NodeId,
    kind: &NodeKind,
    environment: &ParamEnv,
    family_inputs: &[RuntimeValue<B>],
    dynamic_index: Option<&RuntimeValue<B>>,
    choices: &[RuntimeValue<B>],
    repetitions: usize,
) -> Result<f64, RuntimeValueAccessError> {
    if repetitions == 0 {
...
mod tests {
    ...
    fn typed_runtime_input_rejects_wrong_wire_kind_and_times_one_clone() {
        ...
        let inputs = BTreeMap::from([(
            "value".to_owned(),
            RuntimeValue::<CpuDcrtBackend>::Int(BigInt::from(17)),
        )]);
    ...
    fn trapdoor_public_projection_rejects_non_trapdoor_without_secret_access() {
        ...
        let value = RuntimeValue::<CpuDcrtBackend>::Int(BigInt::from(1));
        assert_eq!(project_trapdoor_public(&value), Err(RuntimeValueAccessError::NotTrapdoor));
