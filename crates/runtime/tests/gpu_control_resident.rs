...
use mxx_runtime::{
    ...
    backend::{
        Backend,
        poly::gpu::gpu_backend,
        poly_gpu::{GpuDcrtBackend, GpuFleetMatrix},
    },
    gpu_column_policy::{ColumnCapability, EffectiveGpuOperation, is_resident_control_operation},
};
use num_bigint::{BigInt, Sign};
use std::collections::BTreeMap;

fn control_graph(modulus: BigInt, ring_dimension: usize) -> mxx_dsl::BuiltGraph {
    control_graph_with_count(modulus, ring_dimension, 4)
}
...
fn matrix_partial_slice_concat_replays_with_fresh_owners() {
    ...
    use mxx_primitives::poly::dcrt::gpu::GpuDCRTPoly;
    use std::sync::Arc;
    ...
    let mut plan = runtime
        .plan(
            ...
            &BTreeMap::from([(
                "matrix".into(),
                RuntimeValue::Matrix(Arc::new(GpuFleetMatrix::from_matrix(make(1)))),
            )]),
        )
        .unwrap();
    for scalar in [3, 7] {
        let source = make(scalar);
        let expected = make(2 * scalar).slice(0, 2, 0, 4);
        let result = runtime
            .execute(
                &mut plan,
                BTreeMap::from([(
                    "matrix".into(),
                    RuntimeValue::Matrix(Arc::new(GpuFleetMatrix::from_matrix(source))),
                )]),
                &mut MemoryArtifactStore::default(),
                [scalar as u8; 32],
            )
            .unwrap();
        let RuntimeValue::Matrix(actual) = &result.outputs["matrix"] else {
            panic!("matrix owner")
        };
...
#[test]
#[serial_test::serial]
fn typed_real_boundary_and_trapdoor_public_alias_execute() {
    ...
    for seed in [1, 2] {
        ...
        let RuntimeValue::Trapdoor { public: aliased, .. } = &result.outputs["trapdoor"] else {
            panic!("trapdoor owner")
        };
        assert!(std::sync::Arc::ptr_eq(public, aliased));
    }
}
...
fn resident_pack_invalid_bit_and_out_of_range_suppress_publication() {
    ...
    let ring = Ring::new(modulus.clone(), 8);
    let family =
    ...
    let owner = runtime.backend_mut().integer_values_from_host(&valid).unwrap();
    let mut plan = runtime
        .plan(graph, &BTreeMap::from([("bits".into(), RuntimeValue::integer_values(owner))]))
        .unwrap();
    let owner = runtime.backend_mut().integer_values_from_host(&valid).unwrap();
    runtime
        .execute(
            &mut plan,
            BTreeMap::from([("bits".into(), RuntimeValue::integer_values(owner))]),
            &mut MemoryArtifactStore::default(),
            [0; 32],
        )
    ...
    for invalid_case in 0..3 {
        ...
        let owner = runtime.backend_mut().integer_values_from_host(&bits).unwrap();
        let error = runtime
            .execute(
                &mut plan,
                BTreeMap::from([("bits".into(), RuntimeValue::integer_values(owner))]),
...
fn resident_multiword_polynomial_primitives_rebind_and_decode() {
    ...
    let first_owner = runtime.backend_mut().integer_values_from_host(&first).unwrap();
    let mut plan = runtime
        .plan(
            graph,
            &BTreeMap::from([("values".into(), RuntimeValue::integer_values(first_owner))]),
        )
        .unwrap();
    for shift in [1, 3] {
        let values = make_values(shift);
        let owner = runtime.backend_mut().integer_values_from_host(&values).unwrap();
        let result = runtime
            .execute(
                &mut plan,
                BTreeMap::from([("values".into(), RuntimeValue::integer_values(owner))]),
                &mut MemoryArtifactStore::default(),
                [shift as u8; 32],
            )
            .unwrap();
        ...
        let mut read = |name: &str| {
            let RuntimeValue::IntegerValues(owner) = &result.outputs[name] else {
                panic!("{name} must remain resident")
            };
            runtime.backend_mut().integer_values_to_host(owner).unwrap()
        };
        ...
        let wide = mxx_runtime::backend::poly::threshold_decode_coefficients(
...
fn resident_control_regression_graph_covers_scalar_and_family_operations() {
    let graph = control_graph(BigInt::from(17), 8);
...
fn resident_control_parallel_tail_replays_bounded_wave() {
    ...
    let graph = control_graph_with_count(modulus, parameters.ring_dimension() as usize, 5)
        .validate(&ParamEnv::default())
        .expect("validate tail resident graph");
    let anchor = gpu_anchor(&parameters, &gpu_parameters);
    let mut runtime = GpuRuntime::new(gpu_backend([gpu_parameters])).unwrap();
    let input_values = [1, 2, 3, 4, 5, 6];
    let inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &input_values);
...
fn resident_control_sequential_carried_counts_publish_final_state() {
    ...
    for (nonce, count) in [0usize, 1, 2, 4].into_iter().enumerate() {
        let graph =
            sequential_control_graph(modulus.clone(), parameters.ring_dimension() as usize, count)
                .validate(&ParamEnv::default())
        ...
        let RuntimeValue::IntegerValues(value) = result.outputs["result"].clone() else {
            panic!("sequential resident result must remain device-resident")
        };
        let actual = runtime
            .backend_mut()
            .integer_values_to_host(value.as_ref())
...
fn gpu_control_inputs(
    ...
    anchor: &RuntimeValue<GpuDcrtBackend>,
    values: &[i64],
) -> BTreeMap<String, RuntimeValue<GpuDcrtBackend>> {
    let integer_values = backend
        .integer_values_from_host(&values.iter().copied().map(BigInt::from).collect::<Vec<_>>())
        .expect("upload resident integer family");
    BTreeMap::from([
        ("values".to_owned(), RuntimeValue::integer_values(integer_values)),
...
) -> RuntimeValue<GpuDcrtBackend> {
    let matrix = DCRTPolyMatrix::from_poly_vec_row(
        parameters,
        vec![DCRTPoly::from_usize_to_constant(parameters, 1)],
    );
    RuntimeValue::matrix(GpuFleetMatrix::from_matrix(GpuDCRTPolyMatrix::from_cpu_matrix(
        gpu_parameters,
        &matrix,
    )))
}
...
fn resident_control_replay_uses_new_family_values_and_signed_semantics() {
    ...
    let gpu_parameters =
        GpuDCRTPolyParams::new(parameters.ring_dimension(), moduli, parameters.base_bits(), None);
    let modulus = BigInt::from_biguint(Sign::Plus, parameters.modulus().as_ref().clone());
    let graph = control_graph(modulus, parameters.ring_dimension() as usize)
        .validate(&ParamEnv::default())
    ...
    let planning_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &planning_values);
    let mut plan = runtime.plan(graph, &planning_inputs).expect("prepare resident control plan");

    // Scalar/family nodes are captured into resident CUDA graph regions and do
    // not acquire ordinary fixed-operation plan nodes. Control planning metadata still
    // exposes their shared HostOrControl classification, which lets this
    // integration test verify that no host protocol fallback was selected.
    assert!(
        plan.plan()
            .nodes
            .iter()
            .all(|node| node.effective_operation == EffectiveGpuOperation::HostOrControl &&
                node.column_capability == ColumnCapability::HostOrControl),
        "control graph must not acquire fixed GPU nodes"
    );
    assert!(
        plan.report().stages.iter().any(|stage| {
            stage.effective_operation == EffectiveGpuOperation::HostOrControl &&
                stage.column_capability == ColumnCapability::HostOrControl
        }),
        "resident control graph must have HostOrControl warmup coverage"
    );
    assert!(
        plan.report().stages.iter().all(|stage| {
            stage.effective_operation == EffectiveGpuOperation::HostOrControl &&
                stage.column_capability == ColumnCapability::HostOrControl
        }),
        "control-only graph must not have a non-control or host-staged operation"
    );

    let first_values = [1, 2, 3, 4, 5, 6];
    let first_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &first_values);
    let first = runtime
    ...
    let second_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &second_values);
    let second = runtime
    ...
    assert_eq!(plan.compiled_launch_count(), 0, "control-only replay has no fixed GPU regions");
}

fn resident_output(runtime: &mut GpuRuntime, result: GpuExecutionResult) -> Vec<BigInt> {
    let RuntimeValue::IntegerValues(values) = result.outputs["result"].clone() else {
        panic!("resident control result must remain a device integer owner")
    };
    runtime
        .backend_mut()
        .integer_values_to_host(values.as_ref())
...
fn test_gpu_resident_invalid_index_suppresses_outputs() {
    ...
    let anchor_wire =
        Ring::new(modulus, parameters.ring_dimension() as usize).input("anchor", (1, 1));
    let graph = context
        .output("result", result)
        .unwrap()
    ...
    let anchor = gpu_anchor(&parameters, &gpu_parameters);
    let mut runtime = GpuRuntime::new(gpu_backend([gpu_parameters])).unwrap();
    let planning_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &[0, 1, 0, 1, 0, 0]);
    let mut plan = runtime.plan(graph, &planning_inputs).unwrap();
    let invalid_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &[0, 2, 0, 1, 0, 0]);
    let result =
    ...
    let valid_inputs = gpu_control_inputs(runtime.backend_mut(), &anchor, &[1, 0, 1, 0, 0, 0]);
