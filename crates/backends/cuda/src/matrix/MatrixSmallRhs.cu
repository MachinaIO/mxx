 ...
 */
// Immutable CRT reconstruction metadata belongs to the compact matrix owner.
...
int small_track_consumer(const GpuSmallMatrix *mat, cudaStream_t consumer_stream, cudaEvent_t completion)
{
    ...
    const cudaStream_t producer_stream = mat->stream;
    ...
    bool small)
{
    ...
    const size_t source_limb = slot / digits;
    ...
    const size_t out_row = (range.destination_row + local_row) * (small ? digits : slots) +
        (small ? digit_idx : slot);
    const size_t out_poly = out_row * out_cols + range.destination_column + local_col;
    const size_t out_idx = ((out_poly * n + coeff) * crt_depth +
        (small ? source_limb : 0)) * (1 + magnitude_bytes);
    ...
    size_t source_column_offset)
{
    ...
    const size_t encoded_depth = crt_depth;
    const size_t lower_source_index =
        (((k * source_cols + rhs_col) * static_cast<size_t>(n) + j) * encoded_depth +
            (crt_depth == 1 ? 0 : limb)) * width;
    const size_t upper_source_index = lower_source_index + half * encoded_depth * width;
    ...
    size_t source_column_offset)
{
    ...
    for (uint32_t coefficient = threadIdx.x; coefficient < n; coefficient += blockDim.x)
    {
        const uint8_t *source =
            payload + (((k * source_cols + rhs_col) * static_cast<size_t>(n) + coefficient) * crt_depth +
                (crt_depth == 1 ? 0 : limb)) * width;
    ...
    GpuSmallMatrixBindingDescriptor *out)
{
    ...
    const size_t payload_offset = mat->column_offset * mat->n * mat->crt_depth * coefficient_bytes;
    ...
    *out = GpuSmallMatrixBindingDescriptor{
        ...
        mat->storage_cols,
        mat->column_offset,
        mat->storage_cols * mat->n * mat->crt_depth * coefficient_bytes,
...
extern "C" int gpu_small_matrix_copy(GpuSmallMatrix *out, const GpuSmallMatrix *src)
{
    if (!out || !src || out->ctx != src->ctx || out->rows != src->rows || out->cols != src->cols ||
        ...
        out->bound_domain != src->bound_domain || out->crt_depth != src->crt_depth ||
        out->payload_bytes != src->payload_bytes)
    ...
    const size_t row_bytes = out->cols * out->n * out->crt_depth * (1 + out->magnitude_bytes);
    const size_t out_pitch = out->storage_cols * out->n * out->crt_depth * (1 + out->magnitude_bytes);
    const size_t src_pitch = src->storage_cols * src->n * src->crt_depth * (1 + src->magnitude_bytes);
    auto *destination = out->payload + out->column_offset * out->n * out->crt_depth * (1 + out->magnitude_bytes);
    const auto *source = src->payload + src->column_offset * src->n * src->crt_depth * (1 + src->magnitude_bytes);
...
extern "C" int gpu_small_matrix_copy_cross_context(GpuSmallMatrix *out, const GpuSmallMatrix *src)
{
    if (!out || !src || out->device != src->device || out->rows != src->rows ||
        ...
        out->magnitude_bytes != src->magnitude_bytes ||
        out->bound_domain != src->bound_domain || out->crt_depth != src->crt_depth ||
        out->payload_bytes != src->payload_bytes)
    ...
    size_t source_column_start)
{
    ...
    const size_t column_bytes = out->n * out->crt_depth * coefficient_bytes;
    ...
    GpuSmallMatrix **out)
{
    ...
    *out = nullptr;
    ...
    view->bound_domain = src->bound_domain;
    view->crt_depth = src->crt_depth;
    ...
    const GpuSmallMatrix *mat, uint8_t *payload, size_t payload_len)
{
    ...
    if (payload_len == 0) return 0;
    const size_t coefficient_bytes = 1 + mat->magnitude_bytes;
    const size_t row_bytes = mat->cols * mat->n * mat->crt_depth * coefficient_bytes;
    const size_t source_pitch = mat->storage_cols * mat->n * mat->crt_depth * coefficient_bytes;
    const auto *source = mat->payload + mat->column_offset * mat->n * mat->crt_depth * coefficient_bytes;
    ...
    const MxxDecomposeFragmentRange *ranges)
{
    ...
    const uint64_t expected_bound = (base + 1) / 2;
    if (out->rows < expected_rows || (!ranges && (out->rows != expected_rows || out->cols != src->cols)) || out->bound_words.size() != 1 ||
        ...
        out->bound_domain != (small ? 1u : 0u) ||
        out->crt_depth != (small ? limbs : 1))
    ...
    // An explicit native graph operation uses the builder's dispatch stream.
    cudaStream_t stream = out->stream;
    stream = reinterpret_cast<cudaStream_t>(mxx_gpu_graph_builder_dispatch_stream(
        out->ctx, out->device, reinterpret_cast<void *>(stream)));
    if (!stream) return set_error("missing compact decomposition stream");
    const bool explicit_graph = mxx_gpu_graph_builder_for_stream(
        out->ctx, reinterpret_cast<void *>(stream)) != nullptr;
    ...
    for (size_t block = 0; block < block_count; ++block)
    {
        ...
        if (!explicit_graph &&
            matrix_wait_all_limb_streams(source, out->device, stream, true, true) != 0) return 1;
    ...
    const size_t slots = digits * (small ? limbs : limbs - dropped_moduli);
                    ...
                    static_cast<uint32_t>(poly_count), static_cast<uint32_t>(slots));
    {
        ...
        const int launch_status = mxx_gpu_launch_kernel(src->ctx, stream,
            compact_decompose_kernel, grid, dim3(kSmallThreads), 0,
            explicit_graph ? patches.data() : nullptr,
            explicit_graph ? patches.size() : 0,
            blocks, constants.moduli, out->payload,
            poly_count, out->cols, slots, out->n, digits,
            out->crt_depth, out->magnitude_bytes, base_bits, true, small);
        if (launch_status != 0)
        {
            if (!explicit_graph) (void)small_fence_stream_with_event(stream);
            return launch_status;
        }
    }
    const cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess || (!explicit_graph && small_record(out, stream) != 0))
    {
        (void)small_fence_stream_with_event(stream);
...
extern "C" int gpu_small_matrix_prepare_preimage_hard_cutoff(GpuSmallMatrix *mat)
{
    ...
    if (mat->bound_domain != 0) return set_error("preimage hard-cutoff requires global coefficients");
...
extern "C" int gpu_small_matrix_prepare_preimage_hard_cutoff_for_tile(
    ...
    size_t cols)
{
    if (!mat || mat->bound_domain != 0 || rows == 0 || cols == 0 ||
...
static int gpu_small_matrix_copy_preimage_status_async_impl(GpuSmallMatrix *mat)
{
...
{
    return gpu_small_matrix_copy_preimage_status_async_impl(mat);
}
...
// Synchronous eager API backed by the same fixed staging allocation.
    ...
    size_t fixed_workspace_bytes)
{
    ...
    if (lhs_eval->level < 0 || limbs == 0 || limbs > kMaxSmallLimbCount || out->level != lhs_eval->level ||
        ...
        rhs_small->bound_domain > 1 ||
        rhs_small->crt_depth != (rhs_small->bound_domain == 1 ? limbs : 1) ||
    ...
    for (size_t block = 0; block < source_count; ++block)
    {
        ...
        const bool graph_operation = mxx_gpu_graph_builder_for_stream(
            out->ctx, reinterpret_cast<void *>(stream)) != nullptr;
        if (!graph_operation &&
            (matrix_wait_all_limb_streams(lhs_eval, rhs_small->device, stream, true, true) != 0 ||
    ...
    const bool explicit_graph = mxx_gpu_graph_builder_for_stream(
        out->ctx, reinterpret_cast<void *>(stream)) != nullptr;
    if (explicit_graph && !fixed_workspace)
        return set_error("explicit compact multiplication requires fixed workspace");
    if (!explicit_graph && small_wait(rhs_small, stream) != 0) return 1;
    ...
    auto fail = [&](cudaError_t failure) -> int {
        ...
        const cudaError_t fence_err = explicit_graph ? cudaSuccess : small_fence_stream_with_event(stream);
    ...
    if (fixed_workspace)
    {
        if (fixed_workspace_bytes < expanded_rhs_workspace_bytes)
            return set_error("fixed compact workspace is too small");
        workspace_u32 = static_cast<uint32_t *>(fixed_workspace);
        workspace_u64 = reinterpret_cast<uint64_t *>(
            static_cast<uint8_t *>(fixed_workspace) + workspace_u32_bytes);
    }
    else
    {
    ...
    for (const LimbGroup &group : limb_groups)
    {
        if (n_u32 <= kCompactNttSuffixSize)
        {
            ...
            if (group.narrow)
            {
                ...
                const MxxGraphPatch rhs_patch = small_kernel_pointer_patch(
                    0, 0, rhs_payload_binding_index);
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_all_shared_kernel<uint32_t>, grid,
                    dim3(kSmallThreads), 0,
                    explicit_graph ? &rhs_patch : nullptr, explicit_graph ? 1 : 0,
                ...
                if (launch_status != 0) return fail(cudaErrorInvalidResourceHandle);
            }
            else
            {
                ...
                const MxxGraphPatch rhs_patch = small_kernel_pointer_patch(
                    0, 0, rhs_payload_binding_index);
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_all_shared_kernel<uint64_t>, grid,
                    dim3(kSmallThreads), 0,
                    explicit_graph ? &rhs_patch : nullptr, explicit_graph ? 1 : 0,
                    rhs_small->payload, group_workspace, constants.twiddle_forward,
                    constants.twiddle_shoup_forward, constants.moduli,
        ...
        else
        {
            ...
            if (group.narrow)
            {
                ...
                const MxxGraphPatch rhs_patch = small_kernel_pointer_patch(
                    0, 0, rhs_payload_binding_index);
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_first_kernel<uint32_t>, first_grid,
                    dim3(kSmallThreads), 0,
                    explicit_graph ? &rhs_patch : nullptr, explicit_graph ? 1 : 0,
                    rhs_small->payload, group_workspace, constants.twiddle_forward,
                    constants.twiddle_shoup_forward, constants.moduli,
            ...
            else
            {
                ...
                const MxxGraphPatch rhs_patch = small_kernel_pointer_patch(
                    0, 0, rhs_payload_binding_index);
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_first_kernel<uint64_t>, first_grid,
                    dim3(kSmallThreads), 0,
                    explicit_graph ? &rhs_patch : nullptr, explicit_graph ? 1 : 0,
                    rhs_small->payload, group_workspace, constants.twiddle_forward,
                    constants.twiddle_shoup_forward, constants.moduli,
                 ...
                 len >>= 1)
            {
                ...
                if (group.narrow)
                {
                    uint32_t *group_workspace =
                        workspace_u32 + group.typed_limb_offset * workspace_words_per_limb;
                    const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                        compact_ntt_dif_stage_kernel<uint32_t>, grid,
                        dim3(kSmallThreads), 0, nullptr, 0,
                ...
                else
                {
                    uint64_t *group_workspace =
                        workspace_u64 + group.typed_limb_offset * workspace_words_per_limb;
                    const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                        compact_ntt_dif_stage_kernel<uint64_t>, grid,
                        dim3(kSmallThreads), 0, nullptr, 0,
                        group_workspace, constants.twiddle_forward,
                        constants.twiddle_shoup_forward, constants.moduli,
                        group.limb_offset, group.limb_count, poly_count, n, len);
            ...
            if (group.narrow)
            {
                uint32_t *group_workspace =
                    workspace_u32 + group.typed_limb_offset * workspace_words_per_limb;
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_suffix_kernel<uint32_t>, suffix_grid,
                    dim3(kSmallThreads), 0, nullptr, 0,
                    group_workspace, constants.twiddle_forward,
                    constants.twiddle_shoup_forward, constants.moduli,
            ...
            else
            {
                uint64_t *group_workspace =
                    workspace_u64 + group.typed_limb_offset * workspace_words_per_limb;
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_rhs_dif_suffix_kernel<uint64_t>, suffix_grid,
                    dim3(kSmallThreads), 0, nullptr, 0,
                    group_workspace, constants.twiddle_forward,
                    constants.twiddle_shoup_forward, constants.moduli,
                    group.limb_offset, group.limb_count, poly_count, n_u32);
    ...
    if (err == cudaSuccess)
    {
        for (const LimbGroup &group : limb_groups)
        {
            ...
            if (grid_blocks > std::numeric_limits<uint32_t>::max())
                return fail(cudaErrorInvalidConfiguration);
            const dim3 grid(static_cast<uint32_t>(grid_blocks));
            std::vector<MxxGraphPatch> graph_patches;
            if (explicit_graph)
            {
                graph_patches.reserve(source_count + block_count);
                for (size_t source = 0; source < source_count; ++source)
                    graph_patches.push_back(small_kernel_pointer_patch(
                        0, offsetof(CompactRowBlocks, inputs) + source * sizeof(void *),
                        source_binding_indices[source]));
                for (size_t destination = 0; destination < block_count; ++destination)
                    graph_patches.push_back(small_kernel_pointer_patch(
                        0, offsetof(CompactRowBlocks, outputs) + destination * sizeof(void *),
                        destination_binding_indices[destination]));
            }
            if (group.narrow)
            {
                const uint32_t *group_workspace =
                    workspace_u32 + group.typed_limb_offset * workspace_words_per_limb;
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_accumulate_kernel<uint32_t>, grid, dim3(kSmallThreads), 0,
                    graph_patches.data(), graph_patches.size(),
            ...
            else
            {
                const uint64_t *group_workspace =
                    workspace_u64 + group.typed_limb_offset * workspace_words_per_limb;
                const int launch_status = mxx_gpu_launch_kernel(out->ctx, stream,
                    compact_accumulate_kernel<uint64_t>, grid, dim3(kSmallThreads), 0,
                    graph_patches.data(), graph_patches.size(),
                    blocks, constants.moduli,
                    group.limb_offset, group_workspace, group.limb_count,
                    rows, inner, cols, n, lazy_reduce);
...
extern "C" int gpu_matrix_mul_small_rhs_into_graph(
    GpuMatrix *const *outputs, const GpuMatrix *const *inputs,
    size_t block_count, const GpuSmallMatrix *rhs_small,
    size_t residency_budget_bytes,
    GpuSmallMatrixAllocationReport *allocation_report,
    void *stream, const uint32_t *source_binding_indices,
    const uint32_t *destination_binding_indices,
    uint32_t rhs_payload_binding_index,
    const size_t *block_fragment_ends,
    void *fixed_workspace, size_t fixed_workspace_bytes)
{
    if (!stream || !fixed_workspace || !fixed_workspace_bytes)
        return set_error("missing explicit compact multiplication stream or workspace");
    return gpu_matrix_mul_small_rhs_impl(outputs, inputs, block_count, rhs_small,
        residency_budget_bytes, allocation_report,
        reinterpret_cast<cudaStream_t>(stream), source_binding_indices,
        destination_binding_indices, rhs_payload_binding_index,
        block_fragment_ends, fixed_workspace, fixed_workspace_bytes);
}
