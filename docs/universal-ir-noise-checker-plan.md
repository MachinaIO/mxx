...
| `UniformSample` | Align `ir-core` validation with the distributions the current backend executes: `[-1,1]`, `[0,1]`, and `[0,q-1]`. Analyze the first two with bound one and the full modular range as `NotSmall`. Every other range is a specific runtime-domain validation failure unless a separately reviewed runtime extension adds that distribution. |
