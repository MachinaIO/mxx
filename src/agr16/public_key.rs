...
/// `E(c * s)`, `E(E(c * s) * s)`, `E(s^2)`, and `E(E(s^2) * s)`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Agr16PublicKey<M: PolyMatrix> {
    pub matrix: M,
    pub c_times_s_pubkey: M,
    pub c_times_s_times_s_pubkey: M,
    pub s_square_pubkey: M,
    pub s_square_times_s_pubkey: M,
...
impl<M: PolyMatrix> Agr16PublicKey<M> {
    pub fn new(
        matrix: M,
        c_times_s_pubkey: M,
        c_times_s_times_s_pubkey: M,
        s_square_pubkey: M,
        s_square_times_s_pubkey: M,
    ...
    fn assert_same_s_square_key(&self, other: &Self) {
        ...
        assert_eq!(
            self.s_square_times_s_pubkey, other.s_square_times_s_pubkey,
            "AGR16 public keys must share the same E(s^2) * s advice public key"
        );
    }
...
impl<M: PolyMatrix> Add<&Self> for Agr16PublicKey<M> {
    ...
    fn add(self, other: &Self) -> Self {
        ...
        Self {
            ...
            c_times_s_times_s_pubkey: self.c_times_s_times_s_pubkey +
                &other.c_times_s_times_s_pubkey,
            s_square_pubkey: self.s_square_pubkey,
            s_square_times_s_pubkey: self.s_square_times_s_pubkey,
            reveal_plaintext,
        }
    }
}
...
impl<M: PolyMatrix> Mul<&Self> for Agr16PublicKey<M> {
    ...
    fn mul(self, other: &Self) -> Self {
        ...
        let matrix = (uu.clone() * &self.s_square_pubkey) -
            (other.matrix.clone() * &self.c_times_s_pubkey) -
            (self.matrix.clone() * &other.c_times_s_pubkey);
        let c_times_s_pubkey = (uu * &self.s_square_times_s_pubkey) -
            (other.matrix.clone() * &self.c_times_s_times_s_pubkey) -
            (self.matrix.clone() * &other.c_times_s_times_s_pubkey) -
            (self.c_times_s_pubkey.clone() * &other.c_times_s_pubkey);
        let c_times_s_times_s_pubkey = Self::zero_like(&matrix);
        let reveal_plaintext = self.reveal_plaintext & other.reveal_plaintext;
        Self {
            matrix,
            c_times_s_pubkey,
