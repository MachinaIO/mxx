...
/// shape and instance data remain separate from this diagnostic label.
...
mod tests {
    use super::*;
    use mxx_ir_core::{
        ConcreteRing, ParamEnv, RingExpr, RingRef,
    ...
    fn ring_ref() -> RingRef {
        RingRef::new(RingExpr::Explicit {
            crt_moduli: vec![IntExpr::constant(17)],
            ring_dimension: 8,
        })
    }

    fn concrete_ring() -> ConcreteRing {
        ring_ref().resolve(&ParamEnv::default(), |_, _, _, _| Ok(vec![17])).unwrap()
    }

    fn matrix(rows: usize, columns: usize) -> ConcreteWireType {
        ConcreteWireType::Matrix(ConcreteMatrixType { ring: concrete_ring(), rows, columns })
    }
    ...
    fn every_dsl_primitive_has_an_explicit_gpu_or_resident_lowering() {
        ...
        let primitives = vec![
            ...
            ("modulus_switch", NodeKind::ModulusSwitch { destination: ring_ref() }),
            ("modulus_reduce", NodeKind::ModulusReduce { destination: ring_ref() }),
            ("centered_rebase", NodeKind::CenteredRebase { destination: ring_ref() }),
            ("centered_round_divide", NodeKind::CenteredRoundDivide { divisor: integer.clone() }),
            (
                "rns_mod_up",
                NodeKind::RnsModUp { destination: ring_ref(), digit_size: 1, normalize: false },
            ),
            (
                "rns_mod_down",
                NodeKind::RnsModDown {
                    destination: ring_ref(),
    ...
    fn typed_execution_range_distinguishes_compact_and_tail_fragments() {
        let args = vec![
            ...
            ConcreteWireType::SmallMatrix {
                matrix: ConcreteMatrixType { ring: concrete_ring(), rows: 4, columns: 9 },
