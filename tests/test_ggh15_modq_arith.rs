...
async fn test_ggh15_modq_arith() {
    ...
    let dir_name = format!("test_data/test_ggh15_modq_arith_qlevel_{}", active_q_level);
    ...
    let sample_aux_start = std::time::Instant::now();
    pk_evaluator.sample_aux_matrices(&params);
    info!("sample_aux_matrices elapsed {:?}", sample_aux_start.elapsed());

    let wait_writes_start = std::time::Instant::now();
    wait_for_all_writes(dir.to_path_buf()).await.unwrap();
    info!("wait_for_all_writes elapsed {:?}", wait_writes_start.elapsed());
