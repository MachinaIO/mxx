...
/- The small decomposition keeps one row per digit, combining the same digit
   from every ordered CRT tower into one polynomial.  CRT idempotents preserve
   the independent balanced residue in each tower; this is distinct from the
   regular tower-major decomposition with dropped limbs. -/
noncomputable def smallDigitPoly {q n : Nat} (layout : RegularLayout q)
    (value : ExactPoly layout.ordered.modulusProduct n)
    (digit : Fin layout.digitsPerTower) :
    ExactPoly layout.ordered.modulusProduct n :=
  ∑ tower : Fin layout.crtModuli.length,
    algebraMap (ZMod layout.ordered.modulusProduct)
      (ExactPoly layout.ordered.modulusProduct n)
      (layout.ordered.crtWeight tower) *
      CrtDecomposition.regularDigitExactPoly layout.ordered value ⟨tower, digit⟩

noncomputable def smallDecomposeMatrix {q n rows columns : Nat}
    (layout : RegularLayout q) (target : ExactMatrix q n rows columns) :
    ExactMatrix q n (rows * layout.digitsPerTower) columns :=
  castExactMatrix layout.product_eq.symm
    (fun row column ↦
      let index :=
        (finProdFinEquiv : Fin rows × Fin layout.digitsPerTower ≃
          Fin (rows * layout.digitsPerTower)).symm row
      smallDigitPoly layout (castExactMatrix layout.product_eq target index.1 column) index.2)

def smallGadgetDecomposeRuns {q n rows columns inner : Nat}
    (backend : BackendContext) (base digits : Int)
    (target : ExactMatrix q n rows columns)
    (decomposition : ExactMatrix q n inner columns) : Prop :=
  ∃ layout, backend.regularLayout q n = some layout ∧
    base = layout.base ∧ digits = layout.digitsPerTower ∧
    ∃ hwidth : inner = rows * layout.digitsPerTower,
      decomposition = castMatrixRows hwidth.symm (smallDecomposeMatrix layout target)
