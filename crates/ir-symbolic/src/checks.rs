...
pub use mxx_ir_core::types::NodeId;
use mxx_ir_core::{graph::Graph, types::ConcreteMatrixType};
...
pub enum CheckError {
    #[error(transparent)]
    Core(#[from] mxx_ir_core::checks::CheckError),
...
pub fn check_topological(graph: &Graph) -> Result<(), CheckError> {
    mxx_ir_core::checks::check_topological(graph).map_err(CheckError::Core)
}
...
pub fn check_same_ring(
    ...
    mxx_ir_core::checks::check_same_ring(left, right).map_err(CheckError::Core)
}
...
pub fn check_add_shape(
    ...
    mxx_ir_core::checks::check_add_shape(left, right).map_err(CheckError::Core)
}
...
pub fn multiplication_type(
    ...
    mxx_ir_core::checks::multiplication_type(left, right).map_err(CheckError::Core)
}
