...
fn q_level_from_env() -> Option<usize> {
    std::env::var("LWE_MODQ_ARITH_Q_LEVEL").ok().map(|raw| {
        let level =
            raw.parse::<usize>().expect("LWE_MODQ_ARITH_Q_LEVEL must be a positive integer");
        assert!(level > 0, "LWE_MODQ_ARITH_Q_LEVEL must be greater than or equal to 1");
        level
    })
}

fn height_from_env() -> usize {
    std::env::var("LWE_MODQ_ARITH_HEIGHT")
        .ok()
        .map(|raw| {
            raw.parse::<usize>().expect("LWE_MODQ_ARITH_HEIGHT must be a non-negative integer")
        })
        .unwrap_or(1)
}

fn crt_bits_from_env() -> usize {
    std::env::var("LWE_MODQ_ARITH_CRT_BITS")
        .ok()
        .map(|raw| {
            raw.parse::<usize>().expect("LWE_MODQ_ARITH_CRT_BITS must be a positive integer")
        })
        .unwrap_or(DEFAULT_CRT_BITS)
}

fn base_bits_from_env() -> u32 {
    std::env::var("LWE_MODQ_ARITH_BASE_BITS")
        .ok()
        .map(|raw| raw.parse::<u32>().expect("LWE_MODQ_ARITH_BASE_BITS must be a positive integer"))
        .unwrap_or(DEFAULT_BASE_BITS)
}

fn max_crt_depth_from_env() -> usize {
    std::env::var("LWE_MODQ_ARITH_MAX_CRT_DEPTH")
        .ok()
        .map(|raw| {
            raw.parse::<usize>().expect("LWE_MODQ_ARITH_MAX_CRT_DEPTH must be a positive integer")
        })
        .unwrap_or(DEFAULT_MAX_CRT_DEPTH)
}
...
async fn test_lwe_modq_arith() {
    ...
    let max_crt_depth = max_crt_depth_from_env();
    let num_inputs = 1usize.checked_shl(height as u32).expect("LWE_MODQ_ARITH_HEIGHT is too large");
    let q_level = q_level_from_env();
    let (crt_depth, params, circuit) =
        find_crt_depth_for_modq_arith(q_level, crt_bits, base_bits, max_crt_depth);
    ...
    info!("benchmark tree config: height={} num_inputs={}", height, num_inputs);
    let q_max = *active_q_moduli.iter().max().expect("active_q_moduli must not be empty");
    ...
    let dir_name = format!("test_data/test_lwe_modq_arith_qlevel_{}", active_q_level);
