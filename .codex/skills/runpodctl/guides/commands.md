# Runpodctl Command Examples

Read only the section needed for the requested operation. These are version-dependent examples, not a script to execute in sequence. Check installed `runpodctl <command> --help` before relying on flags. Installation and setup apply only when needed.
...
```bash
runpodctl version                   # Inspect installed version
