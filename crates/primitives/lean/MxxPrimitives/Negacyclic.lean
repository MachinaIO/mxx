...
abbrev RingPoly (n : Nat) (R : Type u) [CommRing R] := Negacyclic n R
abbrev ModMatrix (q n rows columns : Nat) :=
  Matrix (Fin rows) (Fin columns) (RingPoly n (ZMod q))
abbrev IntMatrix (n rows columns : Nat) :=
  Matrix (Fin rows) (Fin columns) (RingPoly n Int)
abbrev ExactPoly (q n : Nat) := RingPoly n (ZMod q)
abbrev ErrorPoly (n : Nat) := RingPoly n Int
