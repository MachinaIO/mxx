    ...
    bool forward)
{
        ...
        for (size_t limb = 0; limb < limb_count; ++limb)
        {
            ...
            if (device < 0)
            {
                ...
                stream = reinterpret_cast<cudaStream_t>(
                    mxx_gpu_graph_builder_dispatch_stream(first->ctx, device,
            ...
            if (!mxx_gpu_graph_builder_for_stream(first->ctx,
                reinterpret_cast<void *>(stream)))
            {
                int status = matrix_wait_limb_stream(source, limb_id, device, stream);
                if (status != 0) return status;
                // Out-of-place transforms consume destination descriptors too.
    ...
    for (size_t offset = 0; offset < matrix_count; offset += kNttBatchMatrices)
    {
        ...
        else
        {
            ...
            if (forward)
            {
                int status = launch_ntt_node(first->ctx, stream, UINT32_MAX,
                    batch_ntt_twist_kernel,
                    coefficient_grid, kTransformThreads, 0, layout, twiddles,
                    twiddle_shoup, constants.moduli, limb_count, n, poly_count);
                if (status != 0) return status;
                status = launch_ntt_node(first->ctx, stream, UINT32_MAX,
                    batch_ntt_bit_reverse_kernel,
                    coefficient_grid, kTransformThreads, 0, layout,
            ...
            if (sources)
            {
                const int status = launch_ntt_node(first->ctx, stream, UINT32_MAX,
                    batch_ntt_first_stage_out_of_place_kernel, stage_grid,
                    kTransformThreads, 0, layout, constants.moduli,
                    limb_count, n, poly_count);
                if (status != 0) return status;
                first_length = 4;
            }
            ...
            {
                const int status = launch_ntt_node(first->ctx, stream, UINT32_MAX,
                    batch_ntt_stage_kernel,
                    stage_grid, kTransformThreads, 0, layout, twiddles,
                    twiddle_shoup, constants.moduli, limb_count, n, len, poly_count);
                if (status != 0) return status;
            }
            if (forward)
            {
            ...
            {
                const int status = launch_ntt_node(first->ctx, stream, UINT32_MAX,
                    batch_ntt_scale_twist_kernel,
                    coefficient_grid, kTransformThreads, 0, layout,
                    twiddles, twiddle_shoup, constants.moduli,
                    constants.n_inv, constants.n_inv_shoup, limb_count, n, poly_count);
                if (status != 0) return status;
            }
        }
