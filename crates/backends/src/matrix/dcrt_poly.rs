...
/// Canonical v1 compact matrix bytes, shared by CPU artifacts and GPU owners.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CompactDcrtMatrixEncoding {
    pub format: u8,
    pub level: usize,
    pub rows: usize,
    pub columns: usize,
...
pub fn decode_compact_dcrt_matrix_bytes(
    bytes: &[u8],
    ring_dimension: usize,
    crt_depth: usize,
) -> Result<CompactDcrtMatrixEncoding, CompactMatrixDecodeError> {
    let (
        (version, format, level, rows, columns, max_coeff_bits, bytes_per_coeff, payload),
        consumed,
    ): ((u8, u8, u32, usize, usize, u16, u16, Vec<u8>), usize) =
        bincode::decode_from_slice(bytes, bincode::config::standard())
            .map_err(|_| CompactMatrixDecodeError::InvalidHeader("cannot decode compact matrix"))?;
    if consumed != bytes.len() {
        return Err(CompactMatrixDecodeError::InvalidHeader("trailing compact matrix bytes"));
    }
    if version != 1 {
        return Err(CompactMatrixDecodeError::UnsupportedVersion {
            version,
            supported_versions: &[1],
        });
    }
    if format > 1 || level as usize >= crt_depth {
        return Err(CompactMatrixDecodeError::InvalidHeader(
            "invalid compact matrix format or level",
        ));
    }
    if bytes_per_coeff as usize != (max_coeff_bits as usize).div_ceil(8) {
        return Err(CompactMatrixDecodeError::InvalidHeader(
    ...
    let coefficient_count = rows
        ...
        .ok_or(CompactMatrixDecodeError::InvalidPayload("compact matrix shape overflows"))?;
    let expected_len =
        coefficient_count.checked_mul(max_coeff_bits as usize).map(|bits| bits.div_ceil(8)).ok_or(
            CompactMatrixDecodeError::InvalidPayload("compact matrix payload length overflows"),
        )?;
    if payload.len() != expected_len {
        return Err(CompactMatrixDecodeError::InvalidPayload(
            "compact matrix payload length mismatch",
        ));
    }
    if let Some(last) = payload.last() {
        let used_bits = coefficient_count * max_coeff_bits as usize % 8;
        if used_bits != 0 && last >> used_bits != 0 {
            return Err(CompactMatrixDecodeError::InvalidPayload("nonzero compact matrix padding"));
        }
    }
    Ok(CompactDcrtMatrixEncoding {
        format,
...
fn compact_bit(payload: &[u8], index: usize) -> bool {
    payload[index / 8] & (1 << (index % 8)) != 0
}
...
impl PolyMatrix for DCRTPolyMatrix {
    ...
    fn into_compact_bytes(self) -> Vec<u8> {
        ...
        let half = modulus.as_ref() >> 1;
        let mut centered =
            Vec::with_capacity(self.nrow * self.ncol * self.params.ring_dimension() as usize);
        let mut maximum_bits = 0usize;
        for row in 0..self.nrow {
            for column in 0..self.ncol {
                for value in self.entry(row, column).coeffs_biguints() {
                    let negative = value > half;
                    let magnitude = if negative { modulus.as_ref() - &value } else { value };
                    maximum_bits = maximum_bits.max(magnitude.bits() as usize);
                    centered.push((negative, magnitude.to_bytes_le()));
                }
            }
        }
        let bit_width = if maximum_bits == 0 { 0 } else { maximum_bits + 1 };
        let bit_width =
            u16::try_from(bit_width).expect("compact coefficient bit width exceeds u16");
        let total_bits = centered
            .len()
            .checked_mul(bit_width as usize)
            .expect("compact matrix payload size overflow");
        ...
        for (index, (negative, magnitude)) in centered.iter().enumerate() {
            ...
            for bit in 0..bit_width.saturating_sub(1) as usize {
                if magnitude.get(bit / 8).is_some_and(|byte| byte & (1 << (bit % 8)) != 0) {
        ...
        bincode::encode_to_vec(
            (
                1u8,
                0u8,
                (self.params.crt_depth() - 1) as u32,
                self.nrow,
                self.ncol,
                bit_width,
    ...
    fn try_from_compact_bytes(
        ...
        let decoded = decode_compact_dcrt_matrix_bytes(
            bytes,
            params.ring_dimension() as usize,
            params.crt_depth(),
        )?;
        if decoded.format != 0 || decoded.level + 1 != params.crt_depth() {
            return Err(CompactMatrixDecodeError::InvalidHeader(
                "CPU matrix artifact requires full coefficient basis",
            ));
        }
        let modulus = params.modulus();
        let half = modulus.as_ref() >> 1;
        let n = params.ring_dimension() as usize;
        let width = decoded.max_coeff_bits as usize;
        let mut matrix = Self::new_empty(params, decoded.rows, decoded.columns);
        for row in 0..decoded.rows {
            for column in 0..decoded.columns {
                let mut coefficients = Vec::with_capacity(n);
                for coefficient in 0..n {
                    let index = (row * decoded.columns + column) * n + coefficient;
                    let base = index * width;
                    let mut magnitude_bytes = vec![0u8; width.div_ceil(8)];
                    for bit in 0..width.saturating_sub(1) {
                        if compact_bit(&decoded.payload, base + bit) {
                            magnitude_bytes[bit / 8] |= 1 << (bit % 8);
                        }
                    }
                    let magnitude = BigUint::from_bytes_le(&magnitude_bytes);
                    let negative = width != 0 && compact_bit(&decoded.payload, base + width - 1);
                    if magnitude > half || (negative && magnitude.is_zero()) {
                        return Err(CompactMatrixDecodeError::InvalidPayload(
                            "noncanonical compact matrix coefficient",
                        ));
                    }
                ...
                matrix.inner[row][column] = DCRTPoly::from_biguints(params, &coefficients);
            }
        }
        Ok(matrix)
    }
    ...
    fn zero_compact_bytes(
        ...
        assert_eq!(level + 1, params.crt_depth(), "CPU compact zero basis mismatch");
...
impl PolyMatrixSmallRhs for DCRTPolyMatrix {
    ...
    fn gadget_decompose(
        ...
        let max_coefficient_bound = (&base + BigUint::from(1u8)) >> 1;
        let value = if small {
            ...
            self.small_decompose_balanced_per_limb()
        } else {
        ...
        if small {
            CpuSmallMatrix::new_per_crt_limb(value, max_coefficient_bound)
        } else {
...
impl DCRTPolyMatrix {
    ...
    /// Balanced digits are computed separately in each ordered CRT tower. A
    /// single logical digit row holds one residue per tower, so multiplying by
    /// the small gadget reconstructs the complete source in every tower.
    fn small_decompose_balanced_per_limb(&self) -> Self {
        let k = self.params.crt_bits().div_ceil(self.params.base_bits() as usize);
        let mut out = Self::new_empty(&self.params, self.nrow * k, self.ncol);
        let f = |row_offsets: Range<usize>, col_offsets: Range<usize>| -> Vec<Vec<DCRTPoly>> {
            let nrow = row_offsets.len();
            ...
            let decomposed_entries: Vec<Vec<Vec<DCRTPoly>>> = parallel_iter!(0..nrow)
                .map(|i| {
                    parallel_iter!(0..ncol)
                        .map(|j| self.dcrt_small_decompose_poly_balanced(&entries[i][j], k))
                        .collect()
                })
                .collect();
            parallel_iter!(0..(nrow * k))
    ...
    fn dcrt_small_decompose_poly_balanced(
        &self,
        poly: &DCRTPoly,
        digit_count: usize,
    ) -> Vec<DCRTPoly> {
        let (moduli, _, _) = self.params.to_crt();
        let modulus = self.params.modulus();
        let base = 1i128.checked_shl(self.params.base_bits()).expect("base bits must fit i128");
        let coefficients = poly.coeffs();
        let all_coefficients = coefficients
            .par_iter()
            .map(|coefficient| {
                let mut residues = vec![Vec::with_capacity(moduli.len()); digit_count];
                for prime in &moduli {
                    let residue = (coefficient.value() % prime).to_u64().unwrap();
                    let mut value = Self::centered_lift_residue(residue, *prime);
                    for digit_residues in &mut residues {
                        let (digit, next) = Self::balanced_digit_step(value, base);
                        digit_residues.push(Self::signed_digit_to_residue(digit, *prime));
                        value = next;
                    }
                    debug_assert_eq!(value, 0, "balanced tower carry must vanish");
                }
                residues
                    .iter()
                    .map(|digit_residues| {
                        Self::crt_reconstruct_residues(&moduli, &modulus, digit_residues)
                            .to_u64_digits()
                    })
                    .collect::<Vec<_>>()
            })
        ...
        (0..digit_count)
            .map(|digit| {
                DCRTPoly::poly_gen_from_vec(
                    &self.params,
                    &all_coefficients
                        .iter()
                        .map(|coefficient| coefficient[digit].clone())
                        .collect::<Vec<_>>(),
                )
            })
            .collect()
...
mod tests {
    ...
    fn cpu_small_matrix_metadata_decomposition_and_multiply_are_typed() {
        ...
        assert_eq!(small.max_coefficient_bound(), &BigUint::from(2u32));
        assert_eq!(small.bound_domain(), CoefficientBoundDomain::PerCrtLimb);
    ...
    #[test]
    fn balanced_small_gadget_decomposition_reconstructs_all_crt_towers() {
        let params = DCRTPolyParams::new(8, 2, 17, 3, None, None);
        ...
        let source = DCRTPolyMatrix::from_poly_vec_row(
            &params,
            vec![DCRTPoly::from_biguints(&params, &coefficients)],
        );
        let digits = params.crt_bits().div_ceil(params.base_bits() as usize);
        let small = source.clone().gadget_decompose(true, Some(digits)).unwrap();
        assert_eq!(small.size(), (digits, 1));
        assert_eq!(small.bound_domain(), CoefficientBoundDomain::PerCrtLimb);
        assert_eq!(small.max_coefficient_bound(), &BigUint::from(4u8));
        let reconstructed = DCRTPolyMatrix::small_gadget_matrix(&params, 1) * small.value().clone();
        assert_eq!(reconstructed.entry(0, 0).coeffs_biguints(), coefficients);

        let payload = small.to_canonical_coefficients().unwrap();
        assert_eq!(payload.len(), digits * 8 * moduli.len() * 2);
        assert!(payload.chunks_exact(2).any(|cell| cell[0] == 2));
        let decoded = CpuSmallMatrix::<DCRTPolyMatrix>::from_canonical_coefficients_in_domain(
            &params,
            digits,
            1,
            small.max_coefficient_bound().clone(),
            CoefficientBoundDomain::PerCrtLimb,
            &payload,
        )
        .unwrap();
        assert_eq!(decoded, small);
        assert_eq!(small.centered_rebase(&params).unwrap(), small);
        let reversed =
            DCRTPolyParams::new(8, 2, 17, 3, Some(moduli.iter().rev().copied().collect()), None);
        assert!(matches!(
            small.centered_rebase(&reversed),
            Err(SmallMatrixError::ParameterMismatch)
        ));
        assert!(
            CpuSmallMatrix::<DCRTPolyMatrix>::from_canonical_coefficients(
    ...
    #[test]
    fn compact_matrix_uses_canonical_centered_bitstream() {
        let params = DCRTPolyParams::new(8, 2, 17, 1, None, None);
        let modulus = params.modulus();
        let values = vec![
            BigUint::ZERO,
            BigUint::from(1u8),
        ...
        let matrix = DCRTPolyMatrix::from_poly_vec(
            &params,
            vec![vec![DCRTPoly::from_biguints(&params, &values)]],
        );
        let bytes = matrix.to_compact_bytes();
        let encoding = decode_compact_dcrt_matrix_bytes(
            &bytes,
            params.ring_dimension() as usize,
            params.crt_depth(),
        )
        ...
        assert_eq!(
            (encoding.format, encoding.level, encoding.rows, encoding.columns),
            (0, 1, 1, 1)
        );
        assert_eq!(
            encoding.bytes_per_coeff as usize,
            (encoding.max_coeff_bits as usize).div_ceil(8)
        );
        assert_eq!(DCRTPolyMatrix::try_from_compact_bytes(&params, &bytes).unwrap(), matrix);

        let zero = DCRTPolyMatrix::zero(&params, 1, 1).to_compact_bytes();
        let empty = decode_compact_dcrt_matrix_bytes(
            &zero,
            params.ring_dimension() as usize,
            params.crt_depth(),
        )
        .unwrap();
        assert_eq!((empty.max_coeff_bits, empty.bytes_per_coeff), (0, 0));
        assert!(empty.payload.is_empty());

        let wrong_format = bincode::encode_to_vec(
            (
                1u8,
                1u8,
                encoding.level as u32,
                encoding.rows,
                encoding.columns,
                encoding.max_coeff_bits,
                encoding.bytes_per_coeff,
                encoding.payload,
            ),
            bincode::config::standard(),
        )
        .unwrap();
        assert!(DCRTPolyMatrix::try_from_compact_bytes(&params, &wrong_format).is_err());
    }
