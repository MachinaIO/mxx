...
{
    template <typename T>
    struct DecomposePointerScratch
    {
        int device;
        const T **d_src;
        T **d_dst;
        ...
        T *d_masks;
        T *d_out_moduli;
        size_t src_ptr_capacity;
        size_t dst_ptr_capacity;
    ...
    template <typename T>
    DecomposePointerScratch<T> &decompose_pointer_scratch_for_device(int device)
    {
        thread_local std::vector<DecomposePointerScratch<T>> scratches;
        ...
        scratches.push_back(DecomposePointerScratch<T>{
            device,
            nullptr,
            nullptr,
            nullptr,
    ...
    template <typename T>
    int ensure_decompose_pointer_scratch(
        int device,
        size_t ptr_count,
        size_t job_count,
        const T ***src_table,
        ...
        T **out_moduli_table)
    {
        if (!src_table || !dst_table || !shift_table || !mask_table || !out_moduli_table)
        {
            return set_error("invalid output tables in ensure_decompose_pointer_scratch");
        }

        auto &scratch = decompose_pointer_scratch_for_device<T>(device);
        int status = ensure_device_pointer_capacity<const T *>(
            device,
            &scratch.d_src,
            &scratch.src_ptr_capacity,
            ptr_count);
        if (status != 0)
        {
            return status;
        }
        status = ensure_device_pointer_capacity<T *>(
            device,
            &scratch.d_dst,
            &scratch.dst_ptr_capacity,
            ptr_count);
        if (status != 0)
        {
            return status;
        ...
        *shift_table = scratch.d_shifts;
        *mask_table = scratch.d_masks;
        ...
        cudaStream_t stream)
    {
        ...
        int status = ensure_decompose_pointer_scratch<T>(
            device,
            ptr_count,
            job_count,
            &d_src,
    ...
    uint32_t base_bits)
{
    ...
    if (out->limb_u64_tables.size() < crt_depth)
    {
        return set_error("unexpected limb table size in gpu_matrix_fill_gadget");
    }
    ...
    for (int limb = 0; limb <= level; ++limb)
    {
        ...
        int status = get_matrix_u64_limb_table(
            out,
            limb,
            &table,
            "unsupported limb type in gpu_matrix_fill_gadget");
        if (status != 0)
        {
            return status;
        }
        if (!table || table->device < 0 || !table->stream)
        {
            return set_error("invalid limb device in gpu_matrix_fill_gadget");
        }
        auto &batch = get_batch(table->device);
        if (!batch.stream)
        {
            batch.stream = table->stream;
        }
        batch.moduli.push_back(out->ctx->moduli[static_cast<size_t>(limb)]);
        batch.limb_indices.push_back(static_cast<uint32_t>(limb));
        batch.dst_ptrs.insert(batch.dst_ptrs.end(), table->entry_ptrs.begin(), table->entry_ptrs.end());
    }
    ...
    return transform_matrix_format_sync(
        out,
        PolyFormat::Eval,
        "failed to transform output matrix to eval in gpu_matrix_fill_gadget");
}
...
extern "C" int gpu_matrix_decompose_base(const GpuMatrix *src, uint32_t base_bits, GpuMatrix *out)
{
    ...
    if (src->format == PolyFormat::Eval)
    {
        ...
        status = transform_matrix_format_sync(
            tmp_inputs_matrix,
            PolyFormat::Coeff,
            "failed to transform clone matrix to coeff in gpu_matrix_decompose_base");
        if (status != 0)
        {
            cleanup_tmp_inputs();
            return status;
    ...
    int transform_status = transform_matrix_format_sync(
        out,
        PolyFormat::Eval,
        "failed to transform output matrix to eval in gpu_matrix_decompose_base");
    ...
    cudaStream_t stream)
{
    ...
    int status = ensure_multi_limb_sampling_scratch(
        device,
        ptr_count,
        limb_count,
        &d_dst,
