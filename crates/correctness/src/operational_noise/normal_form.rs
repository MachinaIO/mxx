...
impl<'a, S: FeasibilitySink> Normalizer<'a, S> {
    ...
    fn observe_coefficient_merge(
        &mut self,
        owner: ScopedExprId,
        left_event: super::g0::EventIndex,
        right_event: super::g0::EventIndex,
        monomial: MonomialId,
    ...
    fn append_contextual_relation_evidence(
        ...
        let trace_owner = if S::ENABLED {
            Some(owner.ok_or(super::g0::G0Error::RelationTraceInvariant)?)
        } else {
            None
        };
        ...
        if let Some(owner) = trace_owner {
            ...
            if coefficient.magnitude() != &BigUint::from(1_u8) {
                ...
                next = BoundValueRef::Transfer(scale_event);
            } else if left_context.is_none() && suffix_context.is_none() && evidence.is_none() {
                let identity_event =
                    self.observe_bound_transfer(owner, BoundRule::Identity { input: next })?;
                next = BoundValueRef::Transfer(identity_event);
            }
        ...
        for (context, left) in [(left_context, true), (suffix_context, false)] {
            ...
            if let Some((owner, current_evidence)) = relation_evidence.take() {
                let context_evidence =
                    self.observe_finite_monomial(owner, context, &BigInt::from(1_u8))?;
                let (left_evidence, right_evidence) = if left {
                    (context_evidence, current_evidence)
                } else {
                    (current_evidence, context_evidence)
                };
    ...
    fn evaluate_nonmatrix(
        ...
        let bound = self.nonmatrix_bound(semantic, expression, node, children)?;
        ...
        match operation {
            MatrixOperation::Add | MatrixOperation::Subtract => {
                ...
                match (left, right) {
                    (Some(left), Some(right)) => {
                        let merge_sources = if S::ENABLED {
                            Some((
                                self.relation_input_result(node.inputs[0])?,
                                self.relation_input_result(node.inputs[1])?,
                            ))
                        } else {
                            None
                        };
            ...
            MatrixOperation::Slice {
                ...
                if let Some(restored) =
                    self.parent_local_slice_nf(scope_proof, semantic, node, children)?
                {
                    return Ok(restored);
                }
                let Some(input) = children.first().and_then(|value| value.exact_nf.as_ref()) else {
    ...
    fn tensor_scalar_action_nf(
        ...
        self.scalar_action_nf(scope_proof, owner, left_expression, right_expression, left, right)
    }
    ...
    fn scalar_action_nf(
        ...
        let ResolvedValueType::Matrix(output_type) =
            self.expressions.value_type(owner.expression())?
        else {
            return Ok(ScalarActionNormalization::NotApplicable);
        };
    ...
    fn parent_local_slice_nf(
        ...
        let ResolvedValueType::Matrix(actual_output) =
            self.expressions.value_type(owner.expression())?
        else {
            return Ok(None);
        };
    ...
    fn add_nf(
        ...
        right: &PolynomialNF,
        subtract: bool,
        merge_sources: Option<(super::g0::EventIndex, super::g0::EventIndex)>,
        ...
        for (id, coefficient) in &right.exact_terms {
            ...
            let collision = terms.contains_key(id);
            let is_zero = {
            ...
            if collision && S::ENABLED {
                let owner = owner.ok_or(super::g0::G0Error::RelationTraceInvariant)?;
                let (left_event, right_event) =
                    merge_sources.ok_or(super::g0::G0Error::RelationTraceInvariant)?;
                self.observe_coefficient_merge(owner, left_event, right_event, *id, signed)?;
            }
        }
        let mut result = PolynomialNF {
            exact_terms: terms,
        ...
        if S::ENABLED &&
            summary_evidence_supplied &&
            evidence.is_none() &&
            (!left.bounded_summary.is_zero() || !right.bounded_summary.is_zero())
        {
            return Err(super::g0::G0Error::RelationTraceInvariant.into());
        }
        if S::ENABLED {
    ...
    fn bound_exact_terms(
        ...
        for (&monomial, coefficient) in &normal_form.exact_terms {
            ...
            if S::ENABLED && matches!(bound, CoefficientBound::Finite(_)) {
                let owner = owner.ok_or(super::g0::G0Error::RelationTraceInvariant)?;
                        ...
                        right.exact_terms.iter().zip(right_source.exact_terms.iter())
                    {
                        self.product_pair_into_body(
                            ...
                            *right_id,
                            ...
                            noise,
                            evidence,
                            &mut pending_relation_applications,
                            &mut worklist,
                        )?;
                    }
                }
    ...
    fn drain_product_worklist<A: ExactTermAccumulator>(
        ...
        while let Some(item) = worklist.pop_front() {
                ...
                ProductWorkItem::GadgetSplice(mut splice) => {
                    ...
                    if let Some(application) = pending_application {
                        ...
                        for ((input_monomial, input_coefficient), replacement) in
                            batch.iter().zip(replacements.iter())
                        {
                            let signed_contribution = &outer_coefficient * input_coefficient;
                            if !signed_contribution.is_zero() {
            ...
            let Some((splice, pending_application)) = self.product_gadget_splice(
                owner,
                monomial,
                coefficient.clone(),
                term_sources,
    ...
    fn merge_product_result_early<A: ExactTermAccumulator>(
        ...
        if S::ENABLED {
            if let Some(sources) = source_refs.take() {
    ...
    fn product_gadget_splice(
        ...
        noise: &mut NumericContract<CoefficientBound>,
        evidence: &mut Option<BoundValueRef>,
    ) -> Result<Option<(ProductGadgetSplice, Option<super::g0::EventIndex>)>, NormalizeError> {
        ...
        for index in 0..ordered_factors.len().saturating_sub(1) {
            ...
            let applied_event = if S::ENABLED {
                let owner = owner.ok_or(super::g0::G0Error::RelationTraceInvariant)?;
                if let Some(sources) = source_refs {
                    self.observe_product_merge(owner, sources, monomial, coefficient.clone())?;
                }
            ...
            if nonzero_summary {
                ...
                *noise = add_noise_summaries(noise, &contextual_bound);
                return Ok(Some((
                    ProductGadgetSplice { left, suffix, input_nf, next_after: None, coefficient },
                    applied_event,
                )));
            }
    ...
    fn observe_finite_monomial(
        ...
        let factors = factor_ids
            .iter()
            .map(|factor| {
                let bound = BoundValueRef::Result {
                    event: self.relation_input_result(factor.expression())?,
                    projection: BoundProjection::Coefficient,
                };
                let facts = self.matrix_value_facts(factor.expression());
                let is_constant_polynomial =
                    facts.is_some_and(|facts| facts.metadata.is_constant_polynomial);
                let support_upper = facts.and_then(|facts| match &facts.polynomial {
                    NumericContract::Known(polynomial) => Some(polynomial.support_upper),
                    NumericContract::Missing => None,
                });
                Ok(MonomialFactorEvidence { bound, is_constant_polynomial, support_upper })
            })
            .collect::<Result<Vec<_>, NormalizeError>>()?;
        if factors.is_empty() {
            return Err(super::g0::G0Error::UnsupportedBoundTransfer.into());
        }
        let monomial_event = self.observe_bound_transfer(
            owner,
    ...
    fn bound_monomial(
        ...
        let NumericContract::Known(product) = self.canonical_monomial_bound(monomial)? else {
            let descriptor = self.monomials.descriptor(monomial)?;
            if descriptor.central_factors.is_empty() && descriptor.ordered_factors.is_empty() {
    ...
    fn canonical_monomial_bound(
        &self,
        monomial: MonomialId,
    ) -> Result<NumericContract<CanonicalMatrixBound>, NormalizeError> {
        let descriptor = self.monomials.descriptor(monomial)?;
    ...
    fn nonmatrix_bound(
        ...
        if S::ENABLED {
            ...
            let rule = match &node.operator {
                ...
                ValueOperator::Scalar(ScalarOperation::Multiply) => BoundRule::Scale {
                    value: Self::predecessor_ref(0, BoundProjection::Coefficient)?,
                    scale: BoundScale::Value(Self::predecessor_ref(
                        1,
                        BoundProjection::Coefficient,
                ...
                _ => return Err(super::g0::G0Error::UnsupportedBoundTransfer.into()),
            };
            self.observe_bound_transfer(owner, rule)?;
        }
        Ok(bound)
    }
...
mod tests {
    ...
    fn summary_contract_fixture() -> (
        ...
        let left = source_with(&mut expressions, matrix_type(), 96_001);
        let right = source_with(&mut expressions, matrix_type(), 96_002);
        let root =
        ...
        let left_monomial = monomials.intern(&expressions, &programs, &[], &[left_scoped]).unwrap();
        let right_monomial =
            monomials.intern(&expressions, &programs, &[], &[right_scoped]).unwrap();
        let left_nf = PolynomialNF {
            exact_terms: BTreeMap::from([(left_monomial, BigInt::from(1_u8))]),
            bounded_summary: BoundedSummary::finite(BoundExpression::new(BigUint::from(2_u8))),
        };
        let right_nf = PolynomialNF {
            exact_terms: BTreeMap::from([(right_monomial, BigInt::from(1_u8))]),
    ...
    fn assert_summary_result_owner(
        ...
        assert_eq!(projection, &BoundProjection::Summary);
        let super::super::g0::NormalizerEvent::Result { owner, .. } =
    ...
    #[test]
    fn direct_product_summary_contract_records_all_ordered_contributions() {
        let (
            mut expressions,
            programs,
            facts,
        ...
        assert!(matches!(bound, NumericContract::Known(CoefficientBound::Finite(_))));
        ...
        let products = trace
            ...
            .filter_map(|event| match event {
                super::super::g0::NormalizerEvent::BoundTransfer {
                    rule: BoundRule::Product { left, right, facts },
                    ..
                } => Some((left.clone(), right.clone(), facts.clone())),
                _ => None,
            })
            .collect::<Vec<_>>();
    ...
    #[test]
    fn direct_tensor_summary_contract_records_all_ordered_contributions() {
        let (
            mut expressions,
            programs,
            facts,
    ...
    #[test]
    fn direct_product_summary_nonunit_weight_has_one_final_scale() {
        let (
            mut expressions,
            programs,
            facts,
        ...
        let scales = trace
            ...
            .filter_map(|(index, event)| match event {
                super::super::g0::NormalizerEvent::BoundTransfer {
                    rule:
                        BoundRule::Scale {
                            value: BoundValueRef::Transfer(value_event),
                            scale: BoundScale::Magnitude(magnitude),
                        },
                    ..
                } => Some((index, value_event, magnitude)),
                _ => None,
            })
            .collect::<Vec<_>>();
    ...
    fn traced_product_summary_rejects_large_or_missing_before_product_output() {
        for (index, large) in [false, true].into_iter().enumerate() {
            let mut expressions = ExprArena::new();
            let mut programs = ProgramArena::new();
            let left_exact = source_with(&mut expressions, matrix_type(), 95_030 + index as u64);
            let left_finite =
                gaussian_factor(&mut expressions, matrix_type(), 95_040 + index as u64, 2);
            let left = expressions
                .intern_matrix_transform(MatrixOperation::Add, &[left_exact, left_finite])
                .unwrap();
            let right = source_with(&mut expressions, matrix_type(), 95_050 + index as u64);
            let root = expressions
    ...
    fn bounded_tensor_is_one_summary_without_exact_terms() {
        ...
        let traced_root = expressions
            ...
            .unwrap();
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, traced_root);
        let mut trace = super::super::g0::FeasibilityTrace::default();
        ...
        assert_eq!(traced_value.exact_nf, value.exact_nf);
        assert_eq!(traced_value.coefficient_bound, value.coefficient_bound);
        assert_eq!(traced_counters, counters);
        assert!(trace.normalization_events().iter().any(|event| {
            matches!(
                event,
                super::super::g0::NormalizerEvent::BoundTransfer {
                    rule: BoundRule::Tensor { .. },
                    ..
                }
            )
        }));
    ...
    #[test]
    fn traced_product_gadget_handoff_records_operator_before_relation() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let scalar = ResolvedMatrixType::new(BigUint::from(17_u8), 1, 1, 1).unwrap();
        let decomposition_type = ResolvedMatrixType::new(BigUint::from(17_u8), 1, 3, 1).unwrap();
        ...
        let operator = events
            ...
            .find_map(|(index, event)| match event {
                NormalizerEvent::CoefficientMerge(CoefficientMerge {
                    ...
                    Some((EventIndex(index as u64), *inputs, *output, signed_contribution.clone()))
                }
                _ => None,
            })
        ...
        let applied = events
            ...
            .find_map(|(index, event)| match event {
                NormalizerEvent::AppliedRelation(observation) if observation.owner == semantic => {
                    Some((EventIndex(index as u64), observation.source_monomial))
                }
                _ => None,
            })
    ...
    fn contextual_gadget_summary_records_ordered_products() {
        ...
        assert_eq!(ordinary_output.exact_terms, enabled_output.exact_terms);
        assert_eq!(ordinary_output.bounded_summary, enabled_output.bounded_summary);
        assert_eq!(ordinary_counters, enabled_counters);
        let applied_event = trace
            .normalization_events()
            .iter()
            .enumerate()
    ...
    fn traced_add_sub_records_same_list_coefficient_merges() {
        for subtract in [false, true] {
            ...
            let output = trace
                ...
                .expect("merge output must be followed by the owner Result");
            ...
            if subtract {
                assert!(!output_nf.exact_terms.contains_key(&merge.output));
            } else {
    ...
    #[test]
    fn traced_add_without_a_term_collision_emits_no_merge() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let left = source_with(&mut expressions, matrix_type(), 10_907);
        let right = source_with(&mut expressions, matrix_type(), 10_908);
        let root =
            expressions.intern_matrix_transform(MatrixOperation::Add, &[left, right]).unwrap();
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
        let mut trace = FeasibilityTrace::default();
        ...
        .normalize(semantic)
        .unwrap();
        assert_eq!(value.exact_nf.as_ref().unwrap().exact_terms.len(), 2);
        assert!(
            !trace
                .normalization_events()
                .iter()
                .any(|event| matches!(event, NormalizerEvent::CoefficientMerge(..)))
        );
        trace.validate_normalization_observations().unwrap();
    }
    ...
    #[test]
    fn traced_crt_summary_evidence_preserves_lane_order_and_nonunit_scale() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let matrix = matrix_type();
        let first = gaussian_factor(&mut expressions, matrix.clone(), 98_201, 2);
        let later = gaussian_factor(&mut expressions, matrix.clone(), 98_202, 5);
        let root = expressions
            .intern_slice(
                ValueOperator::Matrix(MatrixOperation::CrtRecompose {
                    plaintext_moduli: Box::new([BigUint::from(3_u8), BigUint::from(5_u8)]),
        ...
        let root_transfers = trace
            .normalization_events()
            .iter()
            .filter_map(|event| match event {
                NormalizerEvent::BoundTransfer { owner, rule } if *owner == semantic => Some(rule),
                _ => None,
            })
            .collect::<Vec<_>>();
        let scale_indices = trace
            .normalization_events()
            .iter()
            .enumerate()
            .filter_map(|(index, event)| match event {
                ...
                } if *owner == semantic && magnitude == &BigUint::from(3_u8) => Some(index),
                _ => None,
            })
            .collect::<Vec<_>>();
        assert_eq!(scale_indices.len(), 1);
        assert!(!root_transfers.iter().any(|rule| {
            matches!(
                rule,
                BoundRule::Scale { scale: BoundScale::Magnitude(magnitude), .. }
                    if magnitude.is_zero()
            )
        }));
        let scale_index = scale_indices[0];
    ...
    fn traced_concat_summary_evidence_is_ordered_and_has_no_unary_maximum() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let input_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 2).unwrap();
        let output_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 2, 6).unwrap();
        let first_source = source_with(&mut expressions, input_type.clone(), 98_211);
        let first_gaussian = gaussian_factor(&mut expressions, input_type.clone(), 98_212, 3);
        let first = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[first_source, first_gaussian])
            .unwrap();
        let second_source = source_with(&mut expressions, input_type.clone(), 98_213);
        let second_gaussian = gaussian_factor(&mut expressions, input_type.clone(), 98_214, 3);
        let second = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[second_source, second_gaussian])
            .unwrap();
        let third_source = source_with(&mut expressions, input_type.clone(), 98_215);
        let third_gaussian = gaussian_factor(&mut expressions, input_type, 98_216, 3);
        let third = expressions
            .intern_matrix_transform(MatrixOperation::Add, &[third_source, third_gaussian])
            .unwrap();
        ...
        let root = expressions
            .intern_matrix_transform(
                MatrixOperation::Concat {
                    axis: 1,
                    output: output_type,
                    layout: MatrixLayout::row_major(2, 6),
                },
                &inputs,
            )
            .unwrap();
        ...
        let summary_maxima = trace
            .normalization_events()
            .iter()
            .filter_map(|event| match event {
                NormalizerEvent::BoundTransfer { owner, rule: BoundRule::Maximum { inputs } }
                    if *owner == semantic &&
            ...
            .collect::<Vec<_>>();
        assert_eq!(summary_maxima.len(), 1);
        assert_eq!(summary_maxima[0].len(), 3);
        assert!(!trace.normalization_events().iter().any(|event| {
            matches!(
                event,
                NormalizerEvent::BoundTransfer {
                ...
                } if *owner == semantic && inputs.len() < 2
            )
        }));
    }
    ...
    fn shared_summary_consumers_reuse_one_current_frame_result_in_order() {
        ...
        let mut programs = ProgramArena::new();
        let matrix = matrix_type();
        let source = source_with(&mut expressions, matrix.clone(), 98_221);
        let finite = gaussian_factor(&mut expressions, matrix, 98_222, 3);
        let shared =
            expressions.intern_matrix_transform(MatrixOperation::Add, &[source, finite]).unwrap();
        let root =
            expressions.intern_matrix_transform(MatrixOperation::Add, &[shared, shared]).unwrap();
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
        let (ordinary_value, ordinary_counters) = {
            let mut normalizer =
                Normalizer::new(&mut expressions, &programs, &facts, &mut monomials).unwrap();
        ...
        let shared_summary_sums = trace
            ...
            .collect::<Vec<_>>();
        assert_eq!(shared_summary_sums.len(), 1);
        let [left, right] = shared_summary_sums[0].as_ref() else {
            unreachable!("checked binary shared summary sum")
        };
        let (
            BoundValueRef::Result { event: left_event, .. },
            BoundValueRef::Result { event: right_event, .. },
        ) = (left, right)
        else {
    ...
    #[test]
    fn exact_finite_folds_record_monomial_evidence_and_scale_in_order() {
        fn run(
            expressions: &mut ExprArena,
            programs: &mut ProgramArena,
            root: ExprId,
            fact_override: Option<(ExprId, MatrixFacts)>,
        ) -> (AnalyzedValue, NormalizationCounters, FeasibilityTrace) {
            let (mut facts, mut monomials, semantic) = setup(expressions, programs, root);
            if let Some((expression, matrix_facts)) = fact_override {
                facts.insert(expressions, expression, ValueFacts::Matrix(matrix_facts)).unwrap();
            }
            let mut trace = FeasibilityTrace::default();
            let (ordinary, ordinary_counters) = {
                let mut normalizer =
                    Normalizer::new(expressions, programs, &facts, &mut monomials).unwrap();
                let value = normalizer.normalize(semantic).unwrap();
                (value, normalizer.counters())
            };
            let (traced, traced_counters) = {
            ...
            assert!(!trace.normalization_events().iter().any(|event| {
                matches!(
                    ...
                    NormalizerEvent::BoundTransfer {
                        rule: BoundRule::Sum { inputs },
                        ..
                    } if inputs.iter().all(|input| matches!(input, BoundValueRef::Transfer(_)))
                )
            }));
            (traced, traced_counters, trace)
        }

        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let one_by_one = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
        let factor = gaussian_factor(&mut expressions, one_by_one.clone(), 99_001, 3);
        let mut metadata = MatrixMetadata::new(MatrixLayout::row_major(1, 1));
        metadata.is_constant_polynomial = true;
        let mut matrix_facts = MatrixFacts::new(one_by_one.clone(), metadata);
        matrix_facts.coefficient_bound = NumericContract::Known(CoefficientBound::finite(3_u8));
        matrix_facts.polynomial = NumericContract::Known(PolynomialFacts::new(1, 4).unwrap());
        ...
        let (value, _, trace) =
            run(&mut expressions, &mut programs, add, Some((factor, matrix_facts)));
        assert_eq!(value.coefficient_bound, NumericContract::Known(CoefficientBound::finite(3_u8)));
        let (monomial_index, factors) = trace
            .normalization_events()
            .iter()
            .enumerate()
            .find_map(|(index, event)| match event {
            ...
            .expect("one-factor monomial evidence");
        assert_eq!(factors.len(), 1);
        assert!(factors[0].is_constant_polynomial);
        assert_eq!(factors[0].support_upper, Some(1));
        let factor_result = match &factors[0].bound {
            BoundValueRef::Result { event, projection: BoundProjection::Coefficient } => *event,
        ...
        let mut programs = ProgramArena::new();
        let matrix = matrix_type();
        let left = gaussian_factor(&mut expressions, matrix.clone(), 99_002, 2);
        let right = gaussian_factor(&mut expressions, matrix.clone(), 99_003, 3);
        let product =
            expressions.intern_matrix_transform(MatrixOperation::Multiply, &[left, right]).unwrap();
        let (_, _, trace) = run(&mut expressions, &mut programs, product, None);
        let factors = trace
            .normalization_events()
            .iter()
            .find_map(|event| match event {
                NormalizerEvent::BoundTransfer {
                    rule: BoundRule::MonomialProduct { monomial: _, factors },
                    ..
                } => Some(factors),
                _ => None,
            })
            .expect("two-factor monomial evidence");
        assert_eq!(factors.len(), 2);

        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let matrix = matrix_type();
        let factor = gaussian_factor(&mut expressions, matrix.clone(), 99_004, 2);
        let crt = expressions
            .intern_slice(
                ValueOperator::Matrix(MatrixOperation::CrtRecompose {
                    plaintext_moduli: Box::new([BigUint::from(3_u8)]),
                    reconstruction_coefficients: Box::new([BigInt::from(2_u8)]),
                    output: matrix,
                }),
                &[factor],
            )
            .unwrap();
        let (_, _, trace) = run(&mut expressions, &mut programs, crt, None);
        let monomial_index = trace
            .normalization_events()
            .iter()
            .enumerate()
            .find_map(|(index, event)| {
            ...
            .expect("coefficient monomial evidence");
        let scale_index = trace
            .normalization_events()
            .iter()
            .enumerate()
            .find_map(|(index, event)| match event {
                NormalizerEvent::BoundTransfer {
                    rule:
                        BoundRule::Scale {
                            value: BoundValueRef::Transfer(event),
                            scale: BoundScale::Magnitude(magnitude),
                        },
                    ..
                } if *event == EventIndex(monomial_index as u64) &&
                    magnitude == &BigUint::from(2_u8) =>
                {
                    Some(index)
                }
        ...
        let zero_crt = expressions
            .intern_slice(
                ValueOperator::Matrix(MatrixOperation::CrtRecompose {
                    plaintext_moduli: Box::new([BigUint::from(3_u8)]),
                    reconstruction_coefficients: Box::new([BigInt::from(0_u8)]),
                    output: matrix_type(),
                }),
                &[factor],
            )
            .unwrap();
        let (value, _, trace) = run(&mut expressions, &mut programs, zero_crt, None);
    ...
    #[test]
    fn nonzero_factorless_monomial_is_ordinary_only() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let factor = gaussian_factor(&mut expressions, matrix_type(), 99_006, 2);
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, factor);
        let empty = monomials.intern(&expressions, &programs, &[], &[]).unwrap();
        ...
        let mut trace = FeasibilityTrace::default();
        trace.record_invocation_start(semantic).unwrap();
        let mut traced = Normalizer::new_with_sink(
            &mut expressions,
            &programs,
            &facts,
            &mut monomials,
        ...
        .unwrap();
        let mut malformed = PolynomialNF {
            exact_terms: [(empty, BigInt::from(7_u8))].into_iter().collect(),
            bounded_summary: BoundedSummary::zero(),
        };
    ...
    #[test]
    fn monomial_gate_checks_mixed_factor_order_and_arena_liveness() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let scalar_type = ResolvedMatrixType::new(BigUint::from(17_u8), 4, 1, 1).unwrap();
        let matrix_type = matrix_type();
        let central = gaussian_factor(&mut expressions, scalar_type.clone(), 99_007, 2);
        let ordered = gaussian_factor(&mut expressions, matrix_type, 99_008, 3);
        let root = expressions
            .intern_matrix_transform(MatrixOperation::Multiply, &[central, ordered])
            .unwrap();
        let (mut facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, root);
        let mut metadata = MatrixMetadata::new(MatrixLayout::row_major(1, 1));
        metadata.is_constant_polynomial = true;
        let mut central_facts = MatrixFacts::new(scalar_type, metadata);
        ...
        let (event_index, monomial, factors) = trace
            ...
            .find_map(|(index, event)| match event {
                NormalizerEvent::BoundTransfer {
                    ...
                    ..
                } if factors.len() == 2 => Some((index, *monomial, factors.clone())),
                _ => None,
            })
            .expect("mixed central and ordered evidence");
        let descriptor = monomials.descriptor(monomial).unwrap();
        ...
        for (factor, evidence) in expected.iter().zip(&factors) {
            let event = match &evidence.bound {
        ...
        if let Some(NormalizerEvent::BoundTransfer {
            rule: BoundRule::MonomialProduct { factors, .. },
            ..
        }) = swapped.events.iter_mut().find(|event| {
            matches!(event, NormalizerEvent::BoundTransfer {
                rule: BoundRule::MonomialProduct { factors, .. }, ..
            } if factors.len() == 2)
        }) {
            let first_event = match &factors[0].bound {
                BoundValueRef::Result { event, .. } => *event,
                _ => unreachable!(),
            };
            let second_event = match &factors[1].bound {
                BoundValueRef::Result { event, .. } => *event,
                _ => unreachable!(),
            };
            factors[0].bound = BoundValueRef::Result {
                event: second_event,
                projection: BoundProjection::Coefficient,
            };
            factors[1].bound = BoundValueRef::Result {
                event: first_event,
                projection: BoundProjection::Coefficient,
            };
        }
        ...
        *ordered_result.expect("ordered result") = foreign_ordered;
        ...
        if let Some(NormalizerEvent::BoundTransfer {
            ...
            ..
        }) = foreign.events.iter_mut().find(|event| {
            matches!(
                event,
                NormalizerEvent::BoundTransfer { rule: BoundRule::MonomialProduct { .. }, .. }
            )
    ...
    #[test]
    fn negative_monomial_coefficients_use_absolute_scale_only() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let factor = gaussian_factor(&mut expressions, matrix_type(), 99_008, 2);
        let (facts, mut monomials, semantic) = setup(&mut expressions, &mut programs, factor);
        let (value, counters) = {
            let mut normalizer =
                Normalizer::new(&mut expressions, &programs, &facts, &mut monomials).unwrap();
        ...
        .unwrap();
        normalizer.observe_finite_monomial(semantic, monomial, &BigInt::from(-1_i8)).unwrap();
        normalizer.observe_finite_monomial(semantic, monomial, &BigInt::from(-2_i8)).unwrap();
        drop(normalizer);
        trace.record_invocation_end(semantic, &value, &counters).unwrap();
        trace.validate_normalization_observations_with_monomials(&monomials).unwrap();
        assert_eq!(
        ...
        assert!(trace.normalization_events().iter().any(|event| {
            matches!(
                ...
                NormalizerEvent::BoundTransfer {
                    rule: BoundRule::Scale {
                        value: BoundValueRef::Transfer(_),
                        scale: BoundScale::Magnitude(magnitude),
                    },
                    ..
    ...
    #[test]
    fn nonmatrix_bound_rules_preserve_results_and_record_typed_transfers() {
        fn constant(expressions: &mut ExprArena, value: i64) -> ExprId {
            expressions
        ...
        fn root_rule(
            ...
            programs: &mut ProgramArena,
            root: ExprId,
        ) -> (NumericContract<CoefficientBound>, BoundRule) {
            let (facts, mut monomials, semantic) = setup(expressions, programs, root);
            let (ordinary_value, ordinary_counters) = {
                let mut normalizer =
                    Normalizer::new(expressions, programs, &facts, &mut monomials).unwrap();
            ...
            let rule = trace
                ...
                .expect("root bound transfer");
            (traced_value.coefficient_bound, rule)
        }
        ...
        let add = expressions
            .intern(ValueOperator::Scalar(ScalarOperation::Add), Box::new([left, right]))
            .unwrap();
        let (bound, rule) = root_rule(&mut expressions, &mut programs, add);
        assert_eq!(bound, NumericContract::Known(CoefficientBound::finite(5_u8)));
        assert_eq!(
            rule,
            BoundRule::Sum {
                inputs: Box::new([
        ...
        let multiply = expressions
            .intern(ValueOperator::Scalar(ScalarOperation::Multiply), Box::new([left, right]))
            .unwrap();
        let (bound, rule) = root_rule(&mut expressions, &mut programs, multiply);
        assert_eq!(bound, NumericContract::Known(CoefficientBound::finite(6_u8)));
        assert_eq!(
            rule,
            BoundRule::Scale {
                value: BoundValueRef::Predecessor {
        ...
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let left = constant(&mut expressions, 0);
        let right = constant(&mut expressions, 3);
        let zero_multiply = expressions
            .intern(ValueOperator::Scalar(ScalarOperation::Multiply), Box::new([left, right]))
            .unwrap();
        let (bound, _) = root_rule(&mut expressions, &mut programs, zero_multiply);
        assert_eq!(bound, NumericContract::Known(CoefficientBound::ExactZero));

        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let input = constant(&mut expressions, 2);
        let negate = expressions
            .intern(ValueOperator::Scalar(ScalarOperation::Negate), Box::new([input]))
            .unwrap();
        let (bound, rule) = root_rule(&mut expressions, &mut programs, negate);
        assert_eq!(bound, NumericContract::Known(CoefficientBound::finite(2_u8)));
        assert_eq!(
            rule,
            BoundRule::Identity {
                input: BoundValueRef::Predecessor {
    ...
    #[test]
    fn nonmatrix_fact_authority_precedes_operator_and_missing_is_fail_closed() {
        let mut expressions = ExprArena::new();
        let mut programs = ProgramArena::new();
        let source = expressions
            .intern(
                ValueOperator::Source(SemanticSourceIdentity {
                    stable_definition: "scalar-fact".to_owned(),
                    invocation: "scalar-fact".to_owned(),
                    sample_event: None,
                    output_role: "value".to_owned(),
                    sampler: None,
                    artifact: None,
        ...
        let mut scalar_facts = ScalarFacts::new(ResolvedValueType::Int).unwrap();
        scalar_facts.coefficient_bound = NumericContract::Known(CoefficientBound::finite(7_u8));
        facts.insert(&expressions, source, ValueFacts::Scalar(scalar_facts)).unwrap();
        let mut trace = FeasibilityTrace::default();
        let mut normalizer = Normalizer::new_with_sink(
            &mut expressions,
            &programs,
        ...
        let source = expressions
            .intern(
                ValueOperator::Source(SemanticSourceIdentity {
                    stable_definition: "scalar-missing".to_owned(),
                    invocation: "scalar-missing".to_owned(),
                    sample_event: None,
                    output_role: "value".to_owned(),
                    sampler: None,
                    artifact: None,
    ...
    fn run_specialization_cache_fixture(target_bound: i64) {
        ...
        if target_bound != 0 {
            ...
            let operator_index = all_events
                ...
                .expect("product operator merge before universal application");
            assert_eq!(
                operator_index.2,
                match &all_events[applied_index.0 as usize] {
                    NormalizerEvent::AppliedRelation(observation) => observation.source_monomial,
                    _ => unreachable!("universal application index"),
                }
            );
            assert_eq!(operator_index.3, BigInt::from(1_u8));
            assert!(operator_index.0 < applied_index);
            assert!(operator_index.1[0].value_event != operator_index.1[1].value_event);
