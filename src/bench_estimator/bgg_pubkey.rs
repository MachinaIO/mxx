...
mod tests {
    ...
    impl PublicLutSampleAuxBenchEstimator for DummyPublicLutEstimator {
        fn sample_aux_matrices_lut_entry_time(&self) -> CircuitBenchEstimate {
            CircuitBenchEstimate { latency: 3.0, total_time: 5.0 }
        }

        fn sample_aux_matrices_lut_gate_time(&self) -> CircuitBenchEstimate {
            CircuitBenchEstimate { latency: 7.0, total_time: 11.0 }
        }
    ...
    impl SlotTransferSampleAuxBenchEstimator for DummySlotTransferEstimator {
        fn sample_aux_matrices_slot_time(&self) -> CircuitBenchEstimate {
            CircuitBenchEstimate { latency: 19.0, total_time: 23.0 }
        }

        fn sample_aux_matrices_gate_time(&self) -> CircuitBenchEstimate {
            CircuitBenchEstimate { latency: 29.0, total_time: 31.0 }
        }
    ...
    fn test_bgg_pubkey_bench_estimator_uses_requested_formulas_and_measured_gate_times() {
        ...
        assert_eq!(public_lut, CircuitBenchEstimate { total_time: 42.0, latency: 10.0 });

        let slot_transfer = estimator.estimate_slot_transfer_sample_aux_matrices(3, 2);
        assert_eq!(slot_transfer, CircuitBenchEstimate { total_time: 131.0, latency: 48.0 });
