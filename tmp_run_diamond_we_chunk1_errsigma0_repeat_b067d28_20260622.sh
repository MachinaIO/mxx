...
RUN_ID="bench_${POD_ID}_8rtxpro6000_b067d28_diamond_we_chunk1_errsigma0_repeat_w8b1_crt22_log15_iter5_20260622"
...

{
  ...
  echo "commit=${COMMIT}"
  echo "chunk=1"
  echo "error_sigma=0.0"
  echo "repeat_count=3"
  echo "started_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
} > "${STATUS}"

echo -e "timestamp_utc\ttrial\tchunk\terror_sigma\tresult\texit_code\tpeak_memory_used_mib\tdecoded_line\tlog\tvram_status" > "${SUMMARY}"
...
export DIAMOND_WE_GPU_BENCH_ERROR_SIGMA=0.0
